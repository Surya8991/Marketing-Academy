...
9. Practice projects layer (Stage 8, Session 73-76): `/projects` hub live, 60 projects across 32 lessons (Phase 0+1 pilot + Technical SEO Mastery track). 19 more `seo`-track lessons scoped and TODO'd for the next session (PROJECTS_PLAN.md 8.3a, corrected from an initial miscount of 18 — the missed one, `content-decay-refresh`, is only reachable via the non-SEO-named "Content Strategy Mastery" track, don't assume the 4 SEO-named tracks are the complete set) — **use PROJECTS_AUTHORING_GUIDE.md for this, don't re-derive the process**, start with `node --import tsx scripts/get-track-batch-info.mjs <track-slug> --category-only=seo` (once per track, including `content-strategy`). 196 more non-SEO track lessons after that. Centrality/tier data for all 642 lessons in src/lib/projects-assignment.ts (generated).
