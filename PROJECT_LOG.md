...
| 88 | 2026-09-17 | **Quiz UX: always-available retake + results on pass.** `Quiz.tsx` — (1) the "Retake quiz" button now renders on EVERY finished screen (pass, fail, and the mount-restored "Already Passed!" screen), previously it was gated behind `!alreadyPassed` so a passed learner had no way to retake short of a full progress reset; `handleRetry()` also resets the in-memory `alreadyPassed` flag but never clears the persisted pass flag, so a retake can never re-lock an earned lesson. (2) The full per-question review (correct option + explanation) now renders on the pass screen too, not just fail, gated on `!alreadyPassed && selections.length === totalQuestions` and extracted into a shared `reviewList` used by both branches. Verified in-browser on `/learn/seo/core-web-vitals` (5/5 pass shows review + Retake; reload shows "Already Passed!" + Retake; Retake restarts at Q1). `tsc`/lint clean. Docs: README quizzes feature line, AGENTS.md Rule 25 (retake/review notes), this row. |
