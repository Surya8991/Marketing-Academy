...
9. Practice projects layer (Stage 8, Session 73-76): `/projects` hub live, 60 projects across 32 lessons (Phase 0+1 pilot + Technical SEO Mastery track). 18 more `seo`-track lessons scoped and TODO'd for the next session (PROJECTS_PLAN.md 8.3a) — **use PROJECTS_AUTHORING_GUIDE.md for this, don't re-derive the process**, start with `node --import tsx scripts/get-track-batch-info.mjs <track-slug> --category-only=seo`. 196 more non-SEO track lessons after that. Centrality/tier data for all 642 lessons in src/lib/projects-assignment.ts (generated).
