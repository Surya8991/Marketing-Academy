...
9. Practice projects layer (Stage 8, Session 73-76): `/projects` hub live, 60 projects across 32 lessons (Phase 0+1 pilot + Technical SEO Mastery track, the only fully-done track besides Solo Founder's near-complete 17/18). **NEXT UP is fully scoped and measured in PROJECTS_PLAN.md 8.3b** (AI Search Optimization then On-Page SEO Mastery, 27 lessons, owner expanded it to include a full content-quality rewrite of every lesson, not just projects/scenarios). Batch lists, tier pairs, the content audit, the Rule 34 no-new-lessons verdict, and 4 batch-specific hazards are all recorded there, do not re-derive them. Owner set an explicit priority order across ALL 24 tracks (PROJECTS_PLAN.md 8.3a table), not just SEO ones: next up is AI Search Optimization, then AI-First Marketer, Off-Page SEO Mastery, On-Page SEO Mastery, B2B Marketing, Paid Ads Mastery, Data-Driven Marketer, CRO & Conversion Mastery, Analytics & Measurement Mastery, Email & Lifecycle Mastery (owner-specified 1-10), then 13 more tracks in an assistant-proposed order, since confirmed by the owner as-is. Full 1-23 order is locked. **Use PROJECTS_AUTHORING_GUIDE.md for every track, don't re-derive the process**, start with `node --import tsx scripts/get-track-batch-info.mjs <track-slug>` (some tracks span multiple categories, e.g. `ai-search-optimization` = `seo`+`ai-marketing`, `b2b-marketer` = 6 categories — batch/merge per category within the track, not per track). Centrality/tier data for all 642 lessons in src/lib/projects-assignment.ts (generated).
...
12. Concept scenarios (Stage 8.4, Session 76): NEW feature, `src/components/InAction.tsx` (global MDX component, registered in root mdx-components.tsx), embedded directly in lesson MDX right after the heading it illustrates. 24 scenarios live in the Technical SEO Mastery track's 12 lessons. Not build-time-injected (see AGENTS.md Rule 54 for why).
```
17: 
18: **Do NOT:**
