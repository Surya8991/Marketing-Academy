...
| 70 | 2026-08-11 | **Planning session, no feature code.** Wrote `PROJECTS_PLAN.md`, the high-priority roadmap for hands-on projects on all 642 lessons + a `/projects` hub + per-concept scenarios in lessons. Four-agent survey of 454 lessons established a six-mode project model (diagnostic, simulation, build, teardown, drill, calibration) plus an explicit `no-project` verdict. Full mechanical audit of all 642 lesson files recorded as a backlog in plan section 12. **Found a live P0 bug**: `TrackLessonList.tsx` lets a learner complete a whole track, with full XP and a certificate, without opening a lesson or passing a quiz, because it never calls `getQuizPassed()`. `AGENTS.md` Rule 24 corrected (it had documented this as intentional and now contradicted Rule 25); Rules 36-39 added. Bug fix specified in plan 0.1, **not yet applied**. |
