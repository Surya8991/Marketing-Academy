...
9. Practice projects layer (Stage 8, Session 73-77): `/projects` hub live, 88 projects across 45 lessons (Phase 0+1 pilot + Technical SEO Mastery + AI Search Optimization tracks, now 2 fully-done SEO-named tracks plus Solo Founder's near-complete 17/18). **NEXT UP: On-Page SEO Mastery, the second half of PROJECTS_PLAN.md 8.3b** — 5 lessons still need projects (`seo/search-intent`, `seo/image-seo-visual-search`, `seo/video-seo`, `copywriting/headlines`, `content/blog-seo-content`, the last two are cross-category), plus a content-rewrite + `InAction` pass on all 13 of its lessons (8 already have projects from earlier sessions but not yet the rewrite/scenario treatment). A scheduled check-in (`projects-stage-8-3b-checkin`, fires 2026-08-13 03:47 IST) will continue this automatically if no session is active. Batch lists, tier pairs, the content audit, the Rule 34 no-new-lessons verdict, and 4 batch-specific hazards are all recorded in 8.3b, do not re-derive them. Owner set an explicit priority order across ALL 24 tracks (PROJECTS_PLAN.md 8.3a table): AI Search Optimization is now ✅ done, next is AI-First Marketer, Off-Page SEO Mastery, On-Page SEO Mastery (being finished alongside AI Search per 8.3b), B2B Marketing, Paid Ads Mastery, Data-Driven Marketer, CRO & Conversion Mastery, Analytics & Measurement Mastery, Email & Lifecycle Mastery (owner-specified 1-10), then 13 more tracks in an assistant-proposed order, confirmed by the owner as-is. Full 1-23 order is locked. **Use PROJECTS_AUTHORING_GUIDE.md for every track, don't re-derive the process**, start with `node --import tsx scripts/get-track-batch-info.mjs <track-slug>` (some tracks span multiple categories, e.g. `ai-search-optimization` = `seo`+`ai-marketing`, `b2b-marketer` = 6 categories — batch/merge per category within the track, not per track). Centrality/tier data for all 642 lessons in src/lib/projects-assignment.ts (generated). New `src/lib/projects/ai-marketing.ts` category file exists as of Session 77 (first projects in that category, `AI_MARKETING_PROJECTS` export).
...
12. Concept scenarios (Stage 8.4, Session 76-77): `src/components/InAction.tsx` (global MDX component, registered in root mdx-components.tsx), embedded directly in lesson MDX right after the heading it illustrates. 52 scenarios live so far: 24 in the Technical SEO Mastery track's 12 lessons (Session 76) + 28 in the AI Search Optimization track's 14 lessons (Session 77). Not build-time-injected (see AGENTS.md Rule 54 for why).
```
17: 
18: **Do NOT:**
