...
<InAction
  concept="Verifying bot identity instead of trusting the user-agent string"
  companyName="Cloudflare"
  where="investigating widespread reports that an AI crawler kept fetching pages after site owners had blocked it"
  why="publishers wanted proof of who was really behind requests claiming to be legitimate agents, not just what the User-Agent header said"
  what="cross-referenced network-level signals, IP ranges, ASN ownership, and rotating user-agent behavior against declared crawler identities instead of trusting the User-Agent header alone"
  benefit="found the stealth crawling activity spanned tens of thousands of domains and millions of requests per day, evidence strong enough to de-list the crawler as verified and block it network-wide"
  timeframe="within days of opening the investigation"
  date="Aug 2025"
...
<InAction
  concept="Non-Google AI crawler traffic now large enough to matter for capacity planning"
  companyName="Read the Docs"
  where="running a community-funded documentation-hosting platform on a tight bandwidth budget"
  why="AI crawlers were downloading huge volumes of built documentation files, some ignoring crawl-delay and re-fetching the same pages repeatedly"
  what="analyzed server logs, found one crawler alone downloaded 73 TB of zipped HTML in a single month, then blocked abusive AI crawlers by user-agent and IP range"
  benefit="bandwidth for downloaded files dropped 75%, from about 800GB/day to about 200GB/day, saving an estimated $1,500/month in bandwidth costs"
  timeframe="within the month after the block went live"
  date="Jul 2024"
  source="https://about.readthedocs.com/blog/2024/07/ai-crawlers-abuse/"
/>
