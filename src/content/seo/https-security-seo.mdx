...
<InAction
  concept="HTTPS and Browser Trust Signals"
  companyName="Zamberg.com"
  where="an online retailer of premium German grooming tools struggling to earn first-time buyer trust"
  why="new visitors had no way to visually confirm the site was safe to enter payment details on"
  what="Zamberg deployed Comodo Extended Validation SSL certificates and a HackerProof trust seal, both visible browser-level trust signals, across its storefront"
  benefit="an 11% improvement in overall site conversion rate and a 23% increase in average value per transaction"
  timeframe="following the certificate rollout"
  date="2008"
  source="https://www.comodo.com/news/press_releases/28-08-08/"
/>
...
<InAction
  ...
  where="a visual discovery platform migrating its entire site from HTTP to HTTPS"
  why="the signup flow required an HTTP-to-HTTPS redirect hop, and the team wanted the security and speed benefits of a fully encrypted site"
  what="Pinterest's engineering team removed the redirect flow by serving the signup page natively over HTTPS, and outlined plans to add HSTS to prevent SSL stripping on a user's first visit"
  benefit="a 10 percent (max) increase in signups a day, purely from eliminating the extra redirect hop"
  timeframe="immediately after the HTTPS rollout"
  date="Mar 2015"
  source="https://medium.com/pinterest-engineering/making-pinterest-https-637ec925a8ad"
/>
