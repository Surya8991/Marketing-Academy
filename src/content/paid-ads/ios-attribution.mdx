...
<InAction
  ...
  where="Meta (Facebook/Instagram) paid campaigns for vacation bookings"
  why="Post-iOS 14.5 pixel-only tracking was undercounting real bookings and inflating cost per conversion"
  what="Club Med implemented Meta's Conversions API to send booking events server-to-server alongside its existing browser pixel"
  benefit="10% more conversion events were registered and cost per acquisition dropped 26% compared to pixel-only tracking"
  timeframe="After rollout of the CAPI integration, measured against pixel-only campaigns"
  date="2024"
  source="https://www.accuracast.com/case-study/clubmed-capi-gateway/"
/>
