...
<InAction
  concept="Investigating the biggest drop-off with heatmaps and session recordings"
  companyName="Flos USA"
  where="An online lighting retailer selling to US and Canada customers, working with VWO Services on a full-funnel optimization program"
  why="Checkout conversion was underperforming, and the team suspected the problem wasn't only on the checkout page itself"
  what="Used heatmaps, scrollmaps, and session recordings across the whole funnel, not just checkout, to find friction on the homepage and product listing pages that was preventing visitors from ever reaching checkout"
  benefit="125% increase in checkout conversion rate and an 18x return on the optimization program's investment"
  timeframe="18-month full-funnel optimization engagement"
  date="2017"
...
<InAction
  concept="Segmenting a funnel to find a hidden high-converting group"
  companyName="Jitsi (8x8)"
  where="Jitsi.org, the video-conferencing product owned by 8x8, analyzed with Amplitude's funnel and segmentation tools"
  why="The aggregate funnel number looked ordinary and gave no indication that one specific user group was converting far better than the rest"
  what="Segmented the funnel by acquisition path and found that users who installed the Chrome extension converted and retained at meaningfully higher rates than the overall average, then promoted the extension more prominently to steer more users into that path"
  benefit="Doubled Day 7 retention rate after promoting the higher-converting Chrome extension path"
  timeframe="Reported in Amplitude's funnel analysis case study collection"
  date="2024"
...
<ResourceList resources={[
  { title: "GA4 Funnel Exploration Report - Official Guide", url: "https://support.google.com/analytics/answer/9327974", type: "docs", free: true, note: "Google's official guide to building funnel reports in GA4" },
