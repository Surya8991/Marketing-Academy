...
<InAction
  concept="Consent mode and modeled conversions"
  companyName="Nemlig"
  where="Denmark's second-largest e-commerce shop, moving Google tag data collection from the browser to server-side tagging on Google Cloud alongside Consent Mode"
  why="GDPR and the EU e-privacy directive meant a large share of visitors declined cookie consent, leaving gaps in conversion data that were undermining bidding and reporting accuracy"
  what="Implemented server-side tagging with Tag Manager 360, paired with Consent Mode so tag behavior automatically adjusted to each visitor's consent choice while unconsented traffic was filled in with modeled conversions"
  benefit="40% increase in 90-day conversion rates for new customers, plus a 7% improvement in page load speed"
  timeframe="rollout completed within one migration project"
  date="2021"
  source="https://marketingplatform.google.com/about/resources/nemlig-gathers-new-insights-and-grows-conversions/"
/>
...
<InAction
  concept="Server-side tagging and first-party data"
  companyName="Finobo"
  where="A financial services firm helping private clients optimize loans, working with agency MCB"
  why="Their lead journey crossed multiple third-party tools and iframes (calculators, meeting schedulers), and UTM parameters and click IDs were getting dropped along the way, leaving only 10-15% of leads trackable back to a campaign"
  what="Replaced client-side pixels with a dedicated server-side Google Tag Manager container hosted on Stape, forwarding lead events to Meta via the Conversions API so identifiers survived the full multi-domain journey into Salesforce"
  benefit="Lead tracking coverage rose from 10-15% to 80-85%"
  timeframe="single infrastructure migration project"
  date="2024"
  source="https://stape.io/blog/leads-increase-for-finobo-case-study"
/>
