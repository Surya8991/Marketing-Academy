...
<InAction
  concept="Pre-registering sample size and end date before launching a test"
  companyName="Booking.com"
  where="Booking.com's central experimentation platform"
  why="Manually setting a sample size for each of 25,000+ tests a year was not scalable, and inconsistent stopping rules were corrupting the company's decision database over time"
  what="Built automated power analysis that pulls baseline conversion data from the warehouse, calculates the required sample size, and locks a test end date before launch, so no analyst can start a test without one"
  benefit="Runs 25,000+ experiments a year off a single automated sample-size and stopping-rule system, protecting decision quality at a scale where even a 1% false-positive rate would mean roughly 250 wrong calls a year"
  timeframe="Ongoing platform practice, described in the company's published experimentation methodology"
  date="2017"
  source="https://arxiv.org/pdf/1710.08217"
/>
...
<InAction
  concept="Alternative testing methods for low-frequency conversion events"
  companyName="Airbnb"
  where="Airbnb's search ranking experimentation"
  why="Booking is a rare, low-frequency conversion event, so a standard fixed-horizon A/B test needed an impractically large sample to reach significance on ranking changes"
  what="Built an interleaving framework that blends results from two ranking algorithms for the same user in a single session, instead of splitting users into separate control and treatment groups"
  benefit="Reached the same conclusion as the equivalent A/B test using only 4% of the traffic and 0.5% of the running time"
  timeframe="Framework built and adopted starting 2021"
  date="2021"
  source="https://airbnb.tech/data/beyond-a-b-test-speeding-up-airbnb-search-ranking-experimentation-through-interleaving/"
/>
...
<ResourceList resources={[
  ...
  { title: "Alex Molas - Bayesian A/B Testing Is Not Immune to Peeking (2025)", url: "https://www.alexmolas.com/2025/10/30/bayesian-ab-test-peeking.html", type: "article", free: true, note: "Simulation data showing peeking inflates false positives above 40%" },
  ...
  { title: "AB Tasty - A/B Test Sample Size Calculator", url: "https://www.abtasty.com/blog/sample-size-calculator/", type: "article", free: true, note: "Industry benchmark: 30,000 visitors / 3,000 conversions minimum" },
  { title: "Democratizing Online Controlled Experiments at Booking.com", url: "https://arxiv.org/pdf/1710.08217", type: "article", free: true, note: "Booking.com's published experimentation methodology" },
