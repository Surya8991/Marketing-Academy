...
<InAction
  concept="Cohort analysis to find the activation event"
  companyName="Canva"
  where="Canva's growth team, running onboarding experiments with Appcues"
  why="Signup-to-first-design activation was strong but flat, and onboarding treated every new user the same regardless of what they actually wanted to create"
  what="A growth manager surveyed users on what type of poster they wanted to make, identified the six most common intents (Event, Retail, Music, Fundraising, Holiday, Advertising), and tested a personalized onboarding interstitial routed by that intent against a control group"
  benefit="Activation growth rate rose from 10% to 12%, adding more than ten thousand additional activated users per month once rolled out to everyone"
  timeframe="Single onboarding experiment, then rolled out platform-wide"
  date="2022"
  source="https://www.appcues.com/blog/canva-growth-process"
/>
...
<InAction
  concept="Compressing time-to-value with behavior-triggered, personalized nudges"
  companyName="Headspace"
  where="Headspace's lifecycle marketing team, sending rich push notifications and in-app deep links tied to each user's meditation history"
  why="New users needed to reach a first meaningful meditation session fast, but generic time-based reminders weren't accelerating that first win"
  what="Deep-linked push notifications routed users directly to content matched to their preferences instead of a generic app-open reminder, shortening the path from install to first completed session"
  benefit="26% uplift in new-user activation and a 14% uplift in first meditation completion during the campaign period"
