...
<InAction
  concept="Pulling Reach from Real Analytics, Not Memory"
  companyName="Air France-KLM Martinair Cargo"
  where="Air France-KLM's cargo division digital product backlog"
  why="Multiple candidate features competed for the same engineering quarter, and the team needed a defensible way to rank a flight-leg optimization feature against other requests without relying on which department argued loudest"
  what="The team scored candidate features with RICE, pulling Reach from actual operational data on how many flight legs and shipments each feature would touch rather than estimating from memory"
  benefit="The RICE-prioritized flight-leg optimization feature shipped first and reportedly saved the airline's cargo division millions of dollars in fuel costs annually"
  timeframe="Cargo digital product prioritization cycle"
  date="2023"
  source="https://www.productlift.dev/blog/product-prioritization-framework-examples/"
/>
...
<InAction
  concept="RICE Scoring (Reach x Impact x Confidence / Effort)"
  companyName="Intercom"
  where="Intercom's product roadmap prioritization meetings"
  why="The team's existing scoring kept ranking a collaboration feature loved by a small, vocal power-user segment above an onboarding fix that would touch far more new signups"
  what="Product managers added Reach, a real 90-day user count pulled from analytics, as a fourth variable and divided by Effort instead of multiplying by Ease, so a feature's actual audience size could outweigh internal enthusiasm"
  benefit="The RICE-ranked backlog surfaced an onboarding simplification with roughly 40x the user exposure of the previously top-ranked feature, and the framework cut time spent in prioritization meetings roughly in half"
  timeframe="Adopted in 2017, still Intercom's default framework"
  date="2017"
  source="https://whatfix.com/blog/rice-scoring-model/"
/>
