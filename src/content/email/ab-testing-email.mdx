...
<InAction
  concept="Sender Name Testing"
  companyName="MailerLite"
  where="MailerLite's own send-performance research across its customer base"
  why="Marketers assumed the subject line was the only lever worth testing and left the sender field on autopilot as a company name"
  what="Compared campaigns sent from a personal sender name against campaigns sent from a company name, isolating sender identity as the only variable"
  benefit="Personal sender names produced 3.81% more opens than company names"
  timeframe="Findings published in MailerLite's email performance benchmarks report"
  date="2025"
...
<InAction
  concept="CTA (Call to Action) Button testing"
  companyName="Sony"
  where="Promotional banner ad within an email campaign for the VAIO laptop line"
  why="A standard promotional banner ('Shop VAIO laptops') was competing for attention with dozens of other stock offers in the same inbox"
  what="Split-tested a personalized banner CTA reading 'Create your own VAIO laptop' against the generic promotional banner, isolating the CTA copy and framing as the only variable"
  benefit="6% increase in click-through rate and a 21.3% increase in shopping cart additions versus the generic CTA"
  timeframe="Documented as a completed split test"
  date="2024"
  source="https://www.mailerlite.com/blog/ab-testing-examples"
/>
