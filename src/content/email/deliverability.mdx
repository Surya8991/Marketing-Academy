...
<InAction
  concept="DMARC Alignment and Enforcement"
  companyName="Rakuten Card"
  where="Rakuten Card's email domain, a frequent target of phishing campaigns impersonating the brand to Rakuten's Japanese customer base"
  why="Customer support was fielding a steady stream of calls from customers who had received spoofed 'Rakuten Card' phishing emails and could not tell them apart from real account notices"
  what="Rakuten's security team rolled out DMARC on the Rakuten Card domain and moved the policy from monitor-only to enforcement at p=reject, blocking any message that failed SPF or DKIM alignment with the visible From address"
  benefit="Phishing-related customer support call volume dropped by roughly 50% after enforcement went live"
  timeframe="Presented at Rakuten Technology Conference 2019, reflecting the rollout completed that year"
  date="2019"
  source="https://www.slideshare.net/kitaurakenji/rtc-2019-emailsecuritycasestudy"
/>
...
<InAction
  concept="Double Opt-In for List Quality"
  companyName="Mailchimp"
  companyId="mailchimp"
  where="Mailchimp's own platform-wide engagement data, sampled across roughly 30,000 accounts that had each sent at least 10 campaigns, list sizes ranging from 500 to 1.5 million subscribers"
  why="Mailchimp wanted to settle, with its own send data instead of anecdote, whether requiring subscribers to confirm their email address at signup (double opt-in) actually produced a healthier list than accepting addresses immediately (single opt-in)"
  what="Mailchimp compared engagement metrics between accounts using double opt-in confirmation versus single opt-in signup across its full sample"
  benefit="Double opt-in lists showed a 72.2% increase in unique opens, a 75.6% increase in total opens, and a 114% increase in click rates versus single opt-in lists, with meaningfully lower bounce and unsubscribe rates"
  timeframe="Study conducted on 2011 send data, still cited industry-wide as the reference benchmark for opt-in method"
  date="2011"
