...
| # | Item | Section | Status |
|---|---|---|---|
| **9.3** | Non-track lessons in the 8 in-scope categories only (`fundamentals`, `seo`, `paid-ads`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools`) | 0.2 | 🔄 **`paid-ads` category now fully done, 33/33 lessons (Session 86)** — the remaining 18 lessons (native-advertising, mmm-vs-mta, apple-search-ads, ctv-ott-ads, reddit-ads, retail-media, audio-ads, pmax-advantage-plus, demand-gen-campaigns, amazon-advertising, first-party-data-activation, creator-ugc-ads, snapchat-pinterest-ads, ai-max-broad-match, ad-frequency-creative-fatigue, landing-page-message-match, budget-pacing-optimization, ai-creative-testing-at-scale) each got 2 projects + 2 `InAction` scenarios via 9 parallel 2-lesson agent batches, merged with `merge-projects-batch.mjs`. Added 5 real platform tools to `tools-directory.ts` first (Amazon Ads, Apple Search Ads, Pinterest Ads Manager, Snapchat Ads Manager, Reddit Ads Manager — Rule 55), since this category's lessons genuinely needed them. Two mechanical defects caught only by the full verify chain, not the audit script: two agents wrote non-exact `toolName` strings ("Google Ads (Demand Gen campaign type)", "Google Ads (AI Max / broad match)") that failed the audit's exact-match check (fixed via sed to plain "Google Ads"); one agent left a duplicate `mode: "teardown"` object-literal property, a `tsc` hard error not an audit-script check (Rule 68's pattern, recurring). Audit, `tsc`, lint, `npm test` (30/30), `npm run build`, live-checked one lesson + one project page in a fresh browser — all clean. Index regenerated: **529 project cards, 19 category modules.** 8/28 done in `events-experiential` from Session 85 still stands (that category remains out of scope per the owner restriction above). 7 of 8 in-scope categories still remaining: `fundamentals`, `seo`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools`. |
