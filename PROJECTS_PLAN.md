...
| **8.3** | Phase 2, track lessons (240 lessons, ~480 projects) | 6, 0.2 | 🔄 In progress, Session 76-77. Technical SEO Mastery (13/13 ✅, Session 76) + AI Search Optimization (14/14 ✅, Session 77: 24 `seo`-category projects + 4 `ai-marketing`-category projects, new `src/lib/projects/ai-marketing.ts` file). Owner explicitly directed starting Phase 2 before a separate human pilot-walkthrough occurred, superseding this row's prior gate for these two tracks' scope; the gate still applies to the rest of the library. **TODO, not started**: On-Page SEO Mastery's 5 remaining lessons (scoped in 8.3b), Off-Page SEO Mastery, and the rest of the 24-track priority order in 8.3a |
| **8.4** | Phase 2b, concept scenarios, bundled with the 45 stale-year fixes | 10, 12.5 | 🔄 In progress, Session 76-77. Infra built from scratch in Session 76 (none existed before) + 24 scenarios for Technical SEO Mastery + 28 scenarios for AI Search Optimization (Session 77) = 52 shipped. Design deviates from this section's preferred build-time rehype injection — see 8.4a. **TODO, not started**: On-Page SEO Mastery's remaining lessons + the rest of the library (~550-650 more scenarios per this section's original estimate) |
...
| 1 | 🤖 AI-First Marketer | `ai-first-marketer` | 0/14 |
...
> **Status: AI Search Optimization ✅ done (Session 77) — all 14 lessons rewritten + 2 projects + 2 `InAction` scenarios each, verified (`tsc`/`lint`/`npm test` 30/30/`npm run build` all clean) and pushed. On-Page SEO Mastery not started.** Every number below is measured, not estimated — run in Session 76 against live source. Scope was expanded by the owner beyond projects/scenarios to include a **full content-quality rewrite pass on all 27 lessons**, plus adding any genuinely missing related lessons. Owner-chosen sequencing: **AI Search Optimization end-to-end first, then On-Page SEO Mastery.** Two things learned executing the AI Search half, worth carrying into On-Page: (1) an authoring agent can write a lesson's first object key unquoted (`aeo: [` — valid JS, since `aeo` is a legal identifier) instead of quoted (`"aeo": [`) like every other key in the file; harmless to `tsc` but silently miscounts in `merge-projects-batch.mjs`'s regex-based key detector and any other regex-based tooling — see AGENTS.md Rule 58, check for this after every merge. (2) A rewrite agent flagged (but did not fix, out of its scope) a **pre-existing** quiz/lesson mismatch: `seo/zero-click-search`'s quiz option text says "~75-80% of searches" while the lesson itself has always said ~68% — needs a `quizzes.ts` edit, not a content rewrite.
