...
> **✅ PLAN COMPLETE as of Session 86.** The practice-projects layer (Stage 8, all 24 tracks + Stage 8.3b) and Stage 9.3's full owner-narrowed 8-category non-track scope are both fully shipped and verified: 803 project cards across 19 category modules, `tsc`/lint/`npm test`(30/30)/`npm run build` all clean. The only work not covered by this plan's execution order is `events-experiential`'s remaining 20 lessons, which the owner explicitly excluded from scope (Session 85) rather than left incomplete — see the Stage 9.3 section for that directive. This document is kept as the historical record and reference material (Sections 1-18); new project-authoring work should start from `PROJECTS_AUTHORING_GUIDE.md` and AGENTS.md Rules 45-76, not from re-opening this plan's execution order.
>
> **PRIORITY: HIGH.** This is the active roadmap for the next major phase of the project. Section 0 is the running order.
>
> **Status as of Session 86: Stage 8.3a, 8.3b, and Stage 9.3's full 7-category in-scope list (`paid-ads`, `seo`, `product-marketing`, `growth`, `social`, `ai-marketing`, `fundamentals`, `tools`) are ALL complete.** 803 project cards across 19 category modules. The only remaining item anywhere in the original Stage 9 backlog is `events-experiential`'s remaining 20 lessons (Session 85 left it at 8/28, then the owner explicitly took it out of scope for 9.3's narrowed 8-category list — it is not blocked, just not prioritized).
...
| **8.3** | Phase 2, track lessons (240 lessons, ~480 projects) | 6, 0.2 | ✅ **DONE.** All 24 tracks in the 8.3a priority order shipped (Sessions 76-85), plus Stage 9.3's full 8-category non-track backlog shipped on top (Session 86). See Stage 8.3a/8.3b and Stage 9.3 sections below for the per-track/per-category history. Final state: 803 project cards across 19 category modules. |
| **8.4** | Phase 2b, concept scenarios, bundled with the 45 stale-year fixes | 10, 12.5 | ✅ **DONE.** Every track's projects batch also shipped 2 `InAction` concept scenarios per lesson (see 8.3), so this shipped alongside 8.3 rather than as a separate pass. Design deviates from this section's preferred build-time rehype injection — see 8.4a and AGENTS.md Rule 54 for why the direct-MDX-embed approach was used instead. |
...
| — | 📊 Data-Driven Marketer | `data-driven-marketer` | ✅ **19/19, done (Session 85)** |
| — | 🎯 CRO & Conversion Mastery | `cro-mastery` | ✅ **15/15, done (Session 85)** |
| — | 📈 Analytics & Measurement Mastery | `analytics-mastery` | ✅ **15/15, done (Session 85)** |
| — | 📧 Email & Lifecycle Mastery | `email-lifecycle-mastery` | ✅ **15/15, done (Session 85)** |
| — | 🚀 Solo Founder | `solo-founder` | ✅ **18/18, already done (confirmed Session 85, no new authoring needed)** |
...
| — | 🛒 E-commerce Growth | `ecommerce-growth` | ✅ **18/18, done (Session 85)** |
| — | 📱 Social Media Manager | `social-media-manager` | ✅ **14/14, done (Session 85)** |
| — | 🧩 Product Marketing Mastery | `product-marketing-mastery` | ✅ **14/14, done (Session 85)** |
| — | 🎨 Brand Strategy Mastery | `brand-strategy-mastery` | ✅ **14/14, done (Session 85)** |
| — | 🧭 Psychology of Marketing | `psychology-of-marketing` | ✅ **14/14, done (Session 85)** |
| — | 📣 PR & Communications Mastery | `pr-communications-mastery` | ✅ **14/14, done (Session 85)** |

**🎉 Stage 8.3a is fully complete — all 24 tracks in the locked priority order now have practice projects.** 477 project cards across 18 category modules. Proceeding to Stage 9 (long tail) per the standing instruction: 9.1 career layer/portfolio, 9.2 Related Concepts expansion, 9.3 remaining non-track lessons, 9.4 hygiene fixes.
...
**Session 85, Psychology of Marketing track (17th track this session):** first practice-projects content ever authored for the `psychology` category — new stub `src/lib/projects/psychology.ts` created, then 7 parallel 2-lesson batches (A-G) covering all 14 lessons: `psychology-101`, `system-1-system-2`, `maslow-marketing`, `emotion-vs-logic`, `cognitive-biases`, `cialdini-6-principles`, `anchoring-framing`, `loss-aversion`, `scarcity-urgency`, `social-proof-psychology`, `habit-loops`, `behavioral-economics`, `choice-architecture`, `peak-end-rule`. 28 projects merged (14 keys, 2 each) + 28 `InAction` concept scenarios, every scenario backed by a real cited source (Airbnb, Costco, Amazon 100ms study, Slack/Forrester TEI, Harley-Davidson HOG, Dollar Shave Club, Old Spice, Booking.com CMA/CNMC, Amazon FTC Prime settlement, Duolingo streak data, Spotify Wrapped, Leatherman UGC, Fashion Nova FTC settlement, Strava, Peloton, Save More Tomorrow 401(k) study, Opower J-PAL RCT, NYC taxi tipping study, Grubhub FTC settlement, Netflix win-back rate, Apple iPad 2010, The Economist decoy test). Audit, `tsc`, lint, `npm test` (30/30), and `npm run build` all clean on the first merge — no defect classes from Rules 67-71 recurred this batch. Index regenerated: **461 project cards across 18 category modules.**

**Session 85, PR & Communications Mastery track (18th and final track of Stage 8.3a):** grew the existing `src/lib/projects/pr-communications.ts` (created in an earlier session via Off-Page SEO Mastery overlap) from 6 to all 14 lessons — 4 parallel 2-lesson batches (A-D) covering `pr-101`, `earned-owned-paid-media`, `press-release-writing`, `press-kit-media-kit`, `thought-leadership-pr`, `analyst-relations`, `crisis-pr-media-response`, `measuring-pr-impact`. 16 projects merged (8 keys, 2 each) + 16 `InAction` scenarios (Warby Parker, KFC UK "FCK", Duolingo TikTok owl, Chewy Claus campaign, Johnson & Johnson Tylenol 1982, Starbucks 2018 Philadelphia incident, CrowdStrike July 2024 outage, Duolingo DAU data). **Two batches (B and D) stalled silently mid-run with no completion notification** — diagnosed by checking scratch-file/MDX modification timestamps rather than waiting indefinitely, then relaunched scoped to only the missing work (batch D fully retried; batch B retried projects-only since its MDX InAction inserts had already landed, avoiding duplicate inserts). One real defect found at `tsc` time: batch A's `earned-owned-paid-media` lesson had 2 projects each with a duplicate `mode` property (object literal, not a runtime bug, but a `tsc` hard error) — fixed by removing the redundant second assignment in both. Audit, lint, `npm test` (30/30), and `npm run build` all clean after the fix. Index regenerated: **477 project cards across 18 category modules.**

**🎉 Stage 8.3a is now fully complete — all 24 tracks in the locked priority order have practice projects and concept scenarios.** Proceeding directly to Stage 9 per the standing instruction.
...
**Session 85 continued, Data-Driven Marketer (`data-driven-marketer`), 3 categories (`mental-models`, `analytics`, `growth`), 13 remaining lessons:** none of the 13 had `InAction` scenarios yet, so every batch authored both projects and scenarios. Ran `get-track-batch-info.mjs data-driven-marketer` for 7 batches (1 mental-models, 5 analytics, 1 growth), launched 7 parallel agents, merged per category with `merge-projects-batch.mjs` (mental-models: 1 file/2 keys, analytics: 5 files/10 keys, growth: 1 file/1 key). `audit-projects.mjs` clean across all 3 categories. One real defect caught only by `tsc --noEmit`, not the audit script: two agents wrote `archetype: "calibration"` (a valid `ProjectMode` value, but not in the `Archetype` union) instead of a real archetype — fixed by changing both to `"reverse-engineer"` (their `mode: "calibration"` field, the correct place for that value, was already set right). `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix; `build-projects-index.mjs` regenerated (222 project cards). 26 projects + 26 `InAction` scenarios added across `mental-models.ts` (5→7 lessons), `analytics.ts` (3→13 lessons), and `growth.ts` (2→3 lessons). `data-driven-marketer` track is now fully done, 19/19 (the 6 already-done lessons — `base-rates-forecasting`, `analytics-101`, `attribution`, `dark-social`, `north-star-metric`, `marketing-math` — were untouched, already had projects from earlier sessions).

**Session 85 continued, CRO & Conversion Mastery (`cro-mastery`), single category (`cro`), 15 lessons, first projects in this category:** `src/lib/projects/cro.ts` did not exist before this batch, created as a stub (`CRO_PROJECTS: Record<string, Project[]> = {}`, following the naming convention `getProjectByCategoryAndId` expects) before merging, same pattern as `ai-marketing.ts`/`pr-communications.ts`/`product-marketing.ts` in earlier sessions. Ran `get-track-batch-info.mjs cro-mastery` for 8 batches (7×2 lessons + 1×1), launched 8 parallel agents, all authoring both projects and `InAction` scenarios (none of the 15 lessons had scenarios yet). Merged in one `merge-projects-batch.mjs cro` call (15 keys, 30 projects). `audit-projects.mjs` caught one real defect the other checks missed: `friction-audit-session-recording-teardown` had `mode: "teardown"` but populated `steps[]` instead of `teardownItems[]` (the content itself was fine, a diagnostic-shaped exercise, just mismatched mode/array) — fixed by changing `mode` to `"diagnostic"` (its `archetype: "teardown"` stayed correct, per Rule 45's distinction). `tsc --noEmit`, `npm run lint`, `npm test` (30/30 — including the tricky double-hyphen `### Step 2 -- Collapse the Flow` heading slug in `checkout-optimization.mdx`, which one agent independently verified against the real slug algorithm before use), `npm run build` all clean. `build-projects-index.mjs` regenerated (252 project cards). No company used more than 2× across the 30 new projects. `cro-mastery` track is now fully done, 15/15.

**Session 85 continued, Analytics & Measurement Mastery (`analytics-mastery`), single category (`analytics`), only 3 remaining lessons (12 already covered via the Data-Driven Marketer batch's overlap):** `cohort-analysis`, `attribution-models`, `privacy-sandbox`, none with `InAction` scenarios. 2 batches launched; the first attempt was interrupted by a harness process restart mid-run with zero output landed on disk (confirmed via the scratch directory and a fresh `grep -c InAction` on all 3 MDX files before relaunching, per the resume-safety check the tooling instructs), so both batches were relaunched clean rather than resumed against partial state. Merged in one `merge-projects-batch.mjs analytics` call (3 keys, 6 projects, 16 lessons total now in the file). `audit-projects.mjs` clean. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean. `build-projects-index.mjs` regenerated (258 project cards). `analytics-mastery` track is now fully done, 15/15.

**Session 85 continued, Email & Lifecycle Mastery (`email-lifecycle-mastery`), single category (`email`), 12 remaining lessons:** none had `InAction` scenarios yet. Ran `get-track-batch-info.mjs email-lifecycle-mastery` for 6 batches, launched 6 parallel agents. Merged via one `merge-projects-batch.mjs email` call (11 reported keys — a 12th, `segmentation`, was written unquoted by batch A, Rule 58, invisible to the merge script's key-counter but its content DID land in the file since the script pastes whole batch bodies rather than parsing key-by-key; caught by `grep -n "segmentation:"` after the reported count came up short, fixed by quoting the key). Three further defects found only by the verification chain, none by `audit-projects.mjs`'s structural check: (1) `tsc` caught a `paidUpgradeNote` field misplaced inside a `ToolRef` (paid[] item) instead of at the `ToolStack` level, where the type actually declares it; (2) `tsc` caught two more instances of `archetype: "build"` (a valid `ProjectMode` value, not a real `Archetype`) — fixed to `"build-the-asset"`, the same Rule 68 class as Data-Driven Marketer's `"calibration"` instance; (3) `npm test` caught `deliverability-dmarc-rollout-simulation` with `mode: "simulation"` but an entirely empty `stages[]` (the agent wrote a simulation-flavored narrative — weekly DMARC checkpoints — but never actually built the `SimulationStage[]` structure) — fixed by converting it to `mode: "diagnostic"` with 2 real `ProjectStep` entries built from the project's own existing narrative content (its `archetype: "simulation"` was left as-is, a legitimate choice independent of `mode` per Rule 45). `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after all fixes. `build-projects-index.mjs` regenerated (282 project cards). `email-lifecycle-mastery` track is now fully done, 15/15.

**Session 85 continued, Solo Founder (`solo-founder`):** re-ran `get-track-batch-info.mjs solo-founder` and found all 18 lessons already had projects — the plan's stale "17/18" figure had already been closed by category overlap from earlier tracks worked this session (e.g. `keyword-research`, `email-marketing-101`, `analytics-101`, `copywriting-101`). No new authoring needed; confirmed and marked done.

**Session 85 continued, Freelancer & Agency (`freelancer-agency`), 2 categories (`social`, `tools`), only 2 remaining lessons, first projects in BOTH categories:** `instagram` (social) and `all-in-one-tools` (tools). Created stub files `src/lib/projects/social.ts` (`SOCIAL_PROJECTS`) and `src/lib/projects/tools.ts` (`TOOLS_PROJECTS`) before merging, same pattern as `cro.ts` earlier this session. 2 parallel agents, one per category. `audit-projects.mjs` caught 2 real anchor-slug mismatches, both from the site's own slug algorithm being non-obvious rather than a careless mistake: `social.ts` used the raw heading text ("Reels: Format for Reach") instead of a slug at all; `tools.ts` used a double-hyphen slug (`how-it-works--the-playbook`, matching real `github-slugger` output for "How It Works / The Playbook") when the site's own `rehype-slug`-mirroring test/audit logic actually collapses the slash-then-space run to a SINGLE hyphen (`how-it-works-the-playbook`) — confirmed by reading `tests/projects-data.test.ts`'s own `headingIdsFor()` regex rather than guessing, and noted this differs from real `github-slugger`'s behavior (which would produce the double-hyphen version) since the site's test/audit tooling implements its own simplified approximation, not the real library. Both fixed. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean. `build-projects-index.mjs` regenerated (286 project cards, 16 category modules). `freelancer-agency` track is now fully done, 16/16.
...
**Session 85 continued, E-commerce Growth (`ecommerce-growth`), only 1 remaining lesson (17/18 already covered via category overlap from tracks worked earlier this session):** `reengagement`, no `InAction` scenarios. Single agent, merged into `email.ts` (now 17 lessons). `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass, no fixes needed. `build-projects-index.mjs` regenerated (367 project cards, 16 category modules). `ecommerce-growth` track is now fully done, 18/18.

**Session 85 continued, Social Media Manager (`social-media-manager`), 3 categories (`social`, `copywriting`, `tools`), 10 remaining lessons:** `social-strategy-basics`, `linkedin`, `tiktok`, `youtube`, `community-building`, `influencer-marketing`, `social-listening`, `content-calendar` (social), `ad-copy` (copywriting), `hootsuite-buffer` (tools), none with `InAction` scenarios. 6 parallel agents, merged per category (social: 4 files/8 keys, copywriting: 1 file/1 key, tools: 1 file/1 key). `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass, no fixes needed. `build-projects-index.mjs` regenerated (387 project cards, 16 category modules). `social-media-manager` track is now fully done, 14/14.

**Session 85 continued, Product Marketing Mastery (`product-marketing-mastery`), single category, 9 remaining lessons:** `pmm-vs-marketing`, `competitive-intel`, `battlecard-design-adoption`, `sales-enablement`, `pricing-packaging`, `customer-interviews`, `win-loss-analysis`, `analyst-relations`, `category-creation`, none with `InAction` scenarios. 5 parallel agents (one self-caught and fixed a duplicate `stepId` before finishing), merged in one call (9 keys, 18 projects). `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass, no fixes needed. `build-projects-index.mjs` regenerated (405 project cards, 16 category modules). `product-marketing-mastery` track is now fully done, 14/14.

**Session 85 continued, Brand Strategy Mastery (`brand-strategy-mastery`), single category, all 14 lessons, first projects in this category:** `src/lib/projects/brand-strategy.ts` did not exist before this batch, created as a stub (`BRAND_STRATEGY_PROJECTS: Record<string, Project[]> = {}`) before merging, same pattern as `cro.ts`/`social.ts`/`tools.ts` in earlier sessions. Ran `get-track-batch-info.mjs brand-strategy-mastery` for 7 batches, launched 7 parallel agents (one used a `simulation`-mode project for `brand-crisis`). `audit-projects.mjs` caught one real defect: `crisis-war-room-first-four-hours`'s `assess-behind` stage had `lessonAnchor: "mistake-1-the-silence-trap"` pointing at bolded prose text (`**Mistake 1: The Silence Trap**`), not a real `##`/`###` heading — fixed to the real containing heading `common-mistakes`; also improved the same stage's two `lessonRef` values, which had been set to the same slug string instead of the descriptive-prose format every other simulation project in the codebase uses (not itself checked by any test, but inconsistent with the established convention, fixed on sight). `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. `build-projects-index.mjs` regenerated (433 project cards, 17 category modules). `brand-strategy-mastery` track is now fully done, 14/14.
...
**✅ STAGE 9.3'S NARROWED 8-CATEGORY SCOPE IS COMPLETE (Session 86).** `fundamentals`, `seo`, `paid-ads`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools` all fully done — 803 project cards across 19 category modules. See the status row below for the full history.
...
| # | Item | Section | Status |
|---|---|---|---|
| **9.3** | Non-track lessons in the 8 in-scope categories only (`fundamentals`, `seo`, `paid-ads`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools`) | 0.2 | 🎉 **ALL 7 remaining in-scope categories now fully done (Session 86): `seo`, `product-marketing`, `growth`, `social`, `ai-marketing`, `fundamentals`, `tools`, on top of `paid-ads` finished earlier the same session.** Executed as ~85 two-lesson agent batches across the 7 categories, each authoring 2 projects + 2 `InAction` scenarios per lesson, merged category-by-category with `merge-projects-batch.mjs`, then verified with the full `tsc` → lint → `npm test` → `npm run build` chain per category before moving on. **Real gotcha found and documented as AGENTS.md Rule 76**: `scripts/get-category-batch-info.mjs` doesn't resolve `sourceCategory`, so it overcounted `fundamentals`' remaining lessons as 32 when the true count was 19 — 13 of the 32 were the cross-listed `mental-models` lessons (Rule 31/43) already covered under `mental-models.ts`; those were correctly skipped rather than double-authored. Recurring mechanical defects across batches, all caught only by `tsc`/`npm test`, none by the audit script (matches the established Rule 67-70 pattern): duplicate `stepId` written at both the start and end of a `ProjectStep` object (hit in `growth.ts` and `tools.ts`); `paidUpgradeNote` nested inside a `ToolRef` instead of as a `ToolStack` sibling (`ai-marketing.ts`, twice); a non-exact `toolName` string ("Google Ads (...)" variants) failing the audit's exact-match check. 16 more real, previously-uncataloged tools added to `tools-directory.ts` across the session (Rule 55): Amazon Ads, Apple Search Ads, Pinterest Ads Manager, Snapchat Ads Manager, Reddit Ads Manager (from `paid-ads`), PartnerStack, TikTok Shop for Business, Meta Commerce Manager, TINT, Bazaarvoice, HypeAuditor, Grin, Supermetrics, Google Ads Editor, SpyFu, Cursor (from the remaining 7 categories) — `tools-directory.ts` grew from 141 to **157**. Final verified state: **803 project cards, 19 category modules** (up from 477 at the start of Session 86). 8/28 done in `events-experiential` from Session 85 still stands (that category remains out of scope per the owner restriction above, and is the only item left in the original 402-lesson Stage 9.3 backlog that hasn't been addressed one way or another). |
