...
> **The full pre-push gate is `PRE_PUSH_CHECKLIST.md` at the repo root (owner-requested 2026-09-17).** This rule is the docs-update section of it. Work through the whole checklist, the verify chain (`tsc` → `eslint` → `npm test` → `npm run build`, in that order), the `npm ci` lock-file sanity check (a real CI failure this repo hit: a lock file regenerated with a newer local npm than CI's Node 20 / npm 10 resolves optional peers differently and fails CI's `npm ci` while passing locally, regenerate with `npx npm@10 install` and commit it in the same push), the DB-migration and access-control gates, and browser verification, not just this docs subsection, before any push.
...
- **`src/components/Quiz.tsx`**, the quiz section at the bottom of each lesson. On ≥80% correct (4 of 5, since Stage 10.1 — was 3 of 4 before), dispatches `QUIZ_PASSED_EVENT` via `window.dispatchEvent`. `MarkComplete` listens to `QUIZ_PASSED_EVENT` and calls `setQuizPassed(category, slug)` then `handleComplete()`.
  - **A "Retake quiz" button is shown on EVERY finished screen** (pass, fail, and the mount-restored "Already Passed!" screen), so a learner can re-attempt any time without resetting progress. `handleRetry()` never clears the persisted pass flag (`setQuizPassed`), so a retake can never re-lock a lesson already earned, it only reshuffles and resets the in-memory `alreadyPassed` flag for a genuine fresh attempt. Do NOT gate the retake button behind `!alreadyPassed` again; that was the original behavior and it left a passed learner with no way to retake short of a full progress reset.
  - **The full per-question review (correct option + explanation for every question) renders on BOTH the pass and fail screens**, not just fail. It's gated on `!alreadyPassed && selections.length === totalQuestions` (a fresh attempt just completed), so the mount-restored "Already Passed!" screen, which has no per-question answers to show, correctly omits it. Answer-reveal-only-after-submission (Stage 1.2) is unchanged: the review still never appears mid-quiz.
...
### Rule 78 — Accounts/sync final review fixes: user-scoped sync timestamp, lazy DB client, session cookie lookup, and the session-payload allow-list's `role` side effect

The accounts/sync feature (Rule 77) shipped, then a full whole-branch review found two Critical and four Important issues, all fixed in one consolidated pass (PR #23). The corrections change load-bearing behavior described in Rule 77 and the design doc, so they're recorded here rather than silently left for a reader to rediscover.

**1. The local sync timestamp is scoped to the user id that wrote it, not a bare number.** `localStorage`'s `ma_sync_local_updated_at` is `{ userId, at }` JSON, never a plain epoch number. `sync-client.ts`'s `localUpdatedAtFor(userId)` returns `null` (not a stale number) whenever the stored value is absent, malformed, or owned by a different user id — which happens on a shared browser after sign-out, since sign-out deliberately leaves `localStorage` intact. `mergeSnapshots()` branches on that `null` first: a server row existing means **remote always wins**; no server row either means **do nothing** (no restore, no push) rather than risk uploading a guest's or a previous user's leftover data. The accepted tradeoff: a returning user's very first sign-in on a device that already has a server row for their account will have the server row win over same-session local edits made before sign-in — `/settings`'s export/import is the escape hatch for rescuing that local data, not the sync path. Never resurrect the original bare-number-timestamp design; it is a real cross-user data leak on any shared browser, not a hypothetical.

**2. `src/server/db/client.ts`'s `db` export is a lazy Proxy, and so is the `DrizzleAdapter` construction inside `auth.ts` — wrapping only one of the two is not enough.** `@libsql/client`'s `createClient()` for a `file:` URL is eager: it throws synchronously if the target directory doesn't exist, which would break the "zero env vars, nothing changes" guarantee the whole feature promises. A `lazyProxy<T>(factory)` helper defers the real `createClient()`/`drizzle()` call until first property access. That alone isn't sufficient: `DrizzleAdapter(db, ...)` itself reads a property on `db` at construction time (`is(db, BaseSQLiteDatabase)`), so `auth.ts`'s adapter construction must also go through `lazyProxy`, or the eager throw just moves one file over. If a future change touches either `client.ts` or `auth.ts`'s adapter setup, keep both wrapped — testing only "does the app boot with no env vars" against one of the two will pass while the other still explodes.

**3. `src/lib/session-cookie.ts` checks both `__Secure-authjs.session-token` and `authjs.session-token`, never guesses from `NODE_ENV`.** `/api/account/sessions` and `/api/account/delete` both need the current request's own session-token cookie to identify "this session" among the user's listed sessions. A hardcoded single cookie name is wrong in whichever of local-dev/production doesn't match it (Auth.js prefixes the cookie with `__Secure-` only over HTTPS). Use the shared helper, don't re-guess per call site.

**4. `auth.ts`'s `session()` callback constructs an explicit allow-list object, and this has a side effect every future admin-check call site must account for.** The database-session-strategy adapter row previously got spread wholesale into the client-readable `/api/auth/session` JSON, which leaked the raw `sessionToken`. The fix returns `{ expires, user: { id, name, email, image, isAdmin } }` only — deliberately **not** `role`. `isAdmin` is computed server-side via `isAdminUser()` at callback time, so it's still available client-side; `role` itself is not. **Known consequence, parked as an accepted gap, not fixed**: `src/app/api/account/delete/route.ts`'s admin-self-delete guard still reads `session.user.role` (now always `undefined`), silently narrowing that guard to `ADMIN_EMAILS`-only. Low real-world impact today since Marketing Academy has no admin-promotion UI (role and `ADMIN_EMAILS` can't practically diverge here) — but if a future admin UI is added, fix the guard to use `isAdminUser()`/`session.user.isAdmin` before shipping it, don't assume `role` is still on the session payload.

**5. `src/lib/quiz-keys.ts` holds `QUIZ_STORAGE_PREFIX`/`QUIZ_PASS_KEY_PREFIX`/`TRACK_QUIZ_PASS_PREFIX`, moved out of `quizzes.ts`.** `progress-snapshot.ts` needs these prefixes to build `ALLOWED_KEY_PREFIXES`, and it's now reachable from the client bundle via `SyncProvider` in the root layout — importing them from the 2.4MB `quizzes.ts` directly would reintroduce the exact bundle-size risk Rule 41 already warns about. `quizzes.ts` re-exports all three for back-compat; new code should import from `quiz-keys.ts` directly rather than through the re-export.

**Also parked, not fixed, disclosed in the PR description**: `restoreAllKeys()`'s additive (never-deletes) semantics mean a shared-browser sign-out-then-different-sign-in can still leave residual keys from the first user visible to the second in some flows. This needs a real product decision about what sign-out should clear, not a quick patch — do not silently "fix" it without raising it first.

Final shipped test count: **65/65** (`npm test`), up from the pre-feature baseline of 30. `AGENTS.md`'s own rule count and `README.md`'s Key Files table should stay in sync with the two new lib files above whenever either changes again.

### Rule 79: Metadata inheritance: never set a site-wide `alternates.canonical`, and never hardcode `| Marketing Academy` in a page's document `title`

Two related, invisible-in-code SEO defects fixed in Session 88, both a direct consequence of how Next.js App Router **shallow-merges `Metadata` parent→child**. `tsc`/lint/build all pass on both; only viewing the rendered `<head>` (or the browser tab title) reveals them.

**1. Root-layout `alternates.canonical` propagates to every page.** `src/app/layout.tsx`'s `metadata` used to set `alternates: { canonical: BASE }` (the homepage URL). Because a canonical set on the root layout is **inherited by every route that doesn't set its own**, ~all glossary terms, hub/list pages, cheat-sheets, about, resources, tools, etc. emitted `<link rel="canonical" href="https://…/">` pointing at the homepage, telling Google they're duplicates of `/`, a mass de-index signal. **Fixed**: the root layout sets NO `alternates.canonical`; each route declares its own self-canonical (homepage in `page.tsx`, the 15 content pages each set `alternates: { canonical: "/path" }`, dynamic pages set it in their `generateMetadata` return). Lessons/categories/tracks/compare/projects/quizzes already self-canonicalized and were unaffected. **Never add a site-wide canonical to the root layout again**, and when adding a new indexable route, give it its own `alternates.canonical`.

**2. The `title.template` doubles a hardcoded suffix.** `layout.tsx` sets `title: { template: "%s | Marketing Academy", default: "Marketing Academy" }`. The template is applied to any child page whose `title` is a **plain string**, so a page that set `title: "About | Marketing Academy"` rendered `<title>About | Marketing Academy | Marketing Academy</title>` (confirmed live on 30 pages). **Fixed**: page **document** titles are now just the page name (`title: "About"`), letting the template add the brand once. **Do NOT reintroduce the suffix** in a page's document `title`.
- **`openGraph.title` / `twitter.title` are the exception**: the `title.template` does NOT apply to them, so those keep the full branded string (e.g. `openGraph.title: "Practice Projects | Marketing Academy"`). When sweeping titles, edit only the document `title`, not the OG/Twitter ones. Exact-whitespace matching distinguishes them: the document title sits at the metadata-object indent, the OG title one level deeper inside `openGraph: {`.
- A page that genuinely needs a full custom title bypassing the template uses `title: { absolute: "…" }`, not a hardcoded suffix.

### Rule 80: Superadmin (`SUPERADMIN_EMAILS`) is a strictly env-only tier; never persist it to the database, never let a superadmin mutate another superadmin

IMPROVEMENT_PLAN #30 added a second admin tier above the existing DB-persisted `role`/`ADMIN_EMAILS` admin (Rule 78). The entire point of `SUPERADMIN_EMAILS` is that it **cannot be granted or revoked from inside the app**, only by editing Vercel env and redeploying. This only holds if the rule below is never violated:

> **Addendum (Session 89): superadmin is a strict SUPERSET of admin on the session payload.** `auth.ts`'s `session()` callback sets `isAdmin: isAdminUser(u) || isSuperAdminEmail(u.email)` (not `isAdminUser(u)` alone). Without the `|| isSuperAdminEmail` half, a `SUPERADMIN_EMAILS` address that is NOT also in `ADMIN_EMAILS` gets `isAdmin: false`, so the Nav "Admin" link stays hidden AND `requireAdmin()` / the `/admin` page redirect it away, i.e. a superadmin can't reach the very dashboard the tier exists to grant. This does not persist anything (both checks are env-derived at request time), so it does not weaken this rule. Keep the `||`; the Nav gates its admin link on `isAdmin || isSuperAdmin` for the same reason.

**Never write a `role: "superadmin"` value, a `superadmin` column, or any other DB-persisted form of this tier.** `isSuperAdminEmail(email)` (`src/auth.ts`) checks `SUPERADMIN_EMAILS` fresh on every call, no caching, no DB fallback, unlike `isAdminUser()`'s bootstrap-to-`role` pattern. If a future feature needs to "know" superadmin status faster, memoize the env parse (`superAdminEmails` in `src/lib/env.ts` already is, at module scope), don't start writing it to `users`.

**`/api/admin/users/[id]`'s PATCH and DELETE handlers both check `isSuperAdminEmail(target.email)` and refuse the action** if the target is a superadmin, before checking anything else about the target. This means one superadmin can never suspend, demote, or delete another via the dashboard, by design: env-tier accounts are peers, and letting one act on another through the app would recreate exactly the DB-mutable escalation path this tier exists to avoid. The only way to remove a superadmin is to edit `SUPERADMIN_EMAILS` in Vercel and redeploy. Both routes also refuse `target.id === actor.id` (no self-suspend/self-demote/self-delete via this dashboard, self-delete already has its own dedicated flow at `/api/account/delete`).

**Suspension (`users.suspended`) is enforced in three places, not one, miss any of them and a suspended account keeps partial access:**
1. `auth.ts`'s `callbacks.signIn` returns `false` for a suspended user, blocking any NEW sign-in.
2. `auth.ts`'s `callbacks.session` puts `isSuspended` on the session payload (defense in depth for a session created *before* suspension, database sessions live up to 7 days and aren't retroactively invalidated).
3. `requireUser()` (`src/auth.ts`) redirects a suspended session to `/login?suspended=1`, AND the 3 routes that call `auth()` directly instead of `requireUser()`, `/api/sync`, `/api/account/sessions`, `/api/account/delete`, each independently check `isSuspended` before proceeding (`requireUser()`'s redirect doesn't help a `fetch()`-based API route, Rule established by the original sync-route comment in that file).

A new API route that reads `auth()` directly (rather than going through `requireUser()`/`requireAdmin()`/`requireSuperAdmin()`) must add its own `isSuspended` check, or a suspended user keeps that one route's access. Prefer routing through `requireUser()`-family helpers when the route can tolerate a redirect; only reach for a raw `auth()` check when the caller is a `fetch()`-based API client that needs a real 401 instead of a redirect (see `/api/sync/route.ts`'s own comment on why it doesn't use `requireUser()`).

**Every mutating `/api/admin/*` route writes an `adminAuditLog` row via `logAdminAction()` (`src/lib/admin-audit.ts`), this is not optional or best-effort the way the email sends in the same routes are.** The audit log is the only accountability record for a dashboard that can change roles, suspend, or delete accounts; a mutation that succeeds without logging defeats the reason the table exists. There is deliberately no update/delete path exposed for `adminAuditLog` itself, it's append-only.

**`tests/api-auth-coverage.test.ts`'s `AUTH_MARKERS` includes `requireSuperAdmin` alongside `requireUser`/`requireAdmin`/`auth()`**: a route gated only by `requireSuperAdmin()` would otherwise fail that test as "ungated" (it did, until the marker was added; see git history on that file). If a future `require*` helper is added to `src/auth.ts`, add its name to `AUTH_MARKERS` in the same commit, not as an afterthought once CI fails.

### Rule 81: Engagement emails (#28): prefs live in the database, not localStorage; cron triggers are single-day date-equality checks, never ranges

The streak-reminder / resume-learning / weekly-digest emails are the one place in this codebase where the usual "localStorage first, DB is only a signed-in backup" model (Rule 77) doesn't apply, for a simple reason: **a Vercel Cron invocation has no browser attached, so it cannot read anyone's localStorage.** `users.emailStreakReminder`/`emailResumeLearning`/`emailWeeklyDigest` are real DB columns, toggled via `/api/account/email-prefs`, read directly by the three `/api/cron/*` routes. Do not "simplify" this by moving the prefs back into `progress-snapshot.ts`'s localStorage-synced keys, a cron job reading a JSON blob that's only pushed every 2s from an open tab is a much weaker guarantee than a column it can `SELECT` directly, and the whole point of a reminder email is to reach someone who *isn't* currently in a tab.

**`streak-reminder` and `resume-learning` both trigger on an exact date-string equality (`lastActiveDay === daysAgoKey(1)` / `=== daysAgoKey(3)`), never a range (`>= 1 && <= 1` day / `>= 3 && <= 7` days).** This is deliberate and load-bearing, not an oversight: `lastActiveDay` doesn't change while a user stays inactive, so an exact-equality check against "N days ago" is true for exactly one calendar day per lapse, the day after the streak breaks, or the third day of a quiet streak, and false every day before or after. A range would re-fire once per day for the whole window (5 straight "resume learning" emails for a 3-7 day range), which is exactly the nagging behavior the original design explicitly called out as unwanted. If a future engagement email needs a similar "N days since X" trigger, use the same exact-day-equality pattern (`src/lib/cron-dates.ts`'s `daysAgoKey(n)`), not a `>=`/`<=` range, a range needs a separate "already sent" tracking column to avoid repeats; date equality doesn't.

**`src/lib/cron-dates.ts` duplicates engagement.ts's date-key format on purpose.** `engagement.ts`'s `localDate()`/`today()`/`yesterday()` are module-private and not exported, and this codebase has no per-user timezone stored anywhere, the cron job's own server timezone (UTC on Vercel) is what's compared against `lastActiveDay`, which was written in the *browser's* local timezone. This is a known, accepted approximation (a user near the UTC dateline can see a reminder land a few hours off their own "today"), not a bug to silently "fix" by adding timezone plumbing without a product decision first.

**The three cron routes are driven by GitHub Actions (`.github/workflows/engagement-emails.yml`), NOT Vercel Cron.** Vercel's Hobby plan caps cron jobs at 2 (run once/day at Vercel's chosen time), and this project runs three on distinct schedules; GitHub Actions cron is free, unlimited, and honours the exact expressions. `vercel.json` has **no** `crons` key, do not re-add one, it would double-fire every route (a second weekly-digest send with no date gate is a duplicate email to every opted-in user). The workflow's `schedule` block owns the timing; each scheduled run resolves `github.event.schedule` to one route and `curl`s it with the bearer header, and a `workflow_dispatch` input allows a manual single-route fire for testing.

**`isCronAuthorized()` (`src/lib/cron-auth.ts`) checks `Authorization: Bearer $CRON_SECRET`, sent by the GitHub Actions workflow.** This means `CRON_SECRET` must be set in **two** places with the **same** value: as a **Vercel env var** (the route compares `env.CRON_SECRET` server-side) AND as a **GitHub Actions repository secret** (the workflow reads `secrets.CRON_SECRET` to send it). Set one without the other and every cron run 401s. With no `CRON_SECRET` configured at all, every `/api/cron/*` route 401s unconditionally (verified: `isCronAuthorized()` returns `false` immediately when `env.CRON_SECRET` is unset, before even reading the request), so cron sending stays off by default exactly like every other optional feature in this app. `tests/api-auth-coverage.test.ts`'s `AUTH_MARKERS` recognizes `isCronAuthorized(` as a real gate (not an EXEMPT entry), see that file's own comment on why.

**`/api/email/unsubscribe` is the one intentionally public, non-session route added by this feature**: a click from an email client has no session to check. It verifies an HMAC-SHA256 token instead (`src/lib/email/unsubscribe-token.ts`, keyed on `AUTH_SECRET`, no new secret introduced) and only ever flips ONE of the token-holder's own three boolean columns to `false`. It's in `tests/api-auth-coverage.test.ts`'s `EXEMPT` list with a verified reason, not left ungated by accident.
