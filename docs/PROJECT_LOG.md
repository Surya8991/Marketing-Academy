...
> Last audited: 2026-09-17 (Session 89: Nav decluttered into one user menu + mobile Topics/Learn/Resources collapsed into accordions, superadmin admin-link/guard fixed, /profile activity capped at 5 behind "Show more" + heatmap enlarged, /settings masthead, README tool count 157 to 159, and the day's-pushed em dashes removed. All-pages audit: full build prerenders every route, key pages spot-checked clean. See the Session 89 entry below. Earlier this day, Session 88 cont., **Project root reorganized ("organize the project folder files"): this file (and IMPROVEMENT_PLAN.md/PROJECTS_PLAN.md/PROJECTS_AUTHORING_GUIDE.md/BACKLOG.md) moved from the repo root into `docs/`, this file is now `docs/PROJECT_LOG.md`.** `resources/seo-aeo-geo-master-guide.html` → `docs/seo-aeo-geo-master-guide.html`; the old `docs/superpowers/` (dead planning docs from the already-shipped accounts+sync feature) renamed to `docs/archive-superpowers-accounts-sync-2026-08-20/`. `AGENTS.md`/`CLAUDE.md`/`README.md` stay at repo root (`CLAUDE.md`'s `@AGENTS.md` import is location-relative, moving `AGENTS.md` would break it). Zero programmatic references to any of the 5 moved files existed anywhere in the codebase (verified via repo-wide grep first), only prose/comment citations, left as bare filenames rather than sweeping ~150 scattered one-line comments across 50+ files for cosmetic path accuracy. `.vercelignore` simplified (its specific per-file exclusion lines were redundant under the existing broader `/docs/` rule, which now also correctly excludes IMPROVEMENT_PLAN.md, it was never excluded before this). `tsc`/lint clean, 75/75 tests, build passes. If a fresh session's tooling still expects this file at the repo root, it has moved, `docs/PROJECT_LOG.md` is now canonical.)
> Last audited: 2026-09-17 (Session 88 cont., **#28 Re-engagement email, completing IMPROVEMENT_PLAN's Tier 1.** Owner: "continue building #28 re-engagement email." Three opt-in templates (`streak-reminder`, `resume-learning`, `weekly-digest`) + three new `/api/cron/*` routes wired into `vercel.json`'s `crons`, gated by a new `isCronAuthorized()` (`src/lib/cron-auth.ts`) checking Vercel's auto-injected `Authorization: Bearer $CRON_SECRET` header, with no `CRON_SECRET` set, every cron route 401s unconditionally, same "safe with zero new env vars" guarantee the rest of the accounts feature carries. **Real design refinement made mid-build, not anticipated in the original plan sketch**: email prefs live in three new `users` DB columns (`emailStreakReminder`/`emailResumeLearning`/`emailWeeklyDigest`), not localStorage, a scheduled cron function has no browser attached to read from, so the usual "localStorage first, DB only backs up signed-in users" model (Rule 77) doesn't fit here. New Settings "Email Notifications" card (signed-in only, all default OFF), reads/writes via `/api/account/email-prefs`. `streak-reminder`/`resume-learning` both trigger on an EXACT date-string equality (`lastActiveDay === daysAgoKey(1)` / `=== daysAgoKey(3)`), never a range, naturally fires exactly once per lapse with no "already sent" tracking column needed; a range would re-send daily for the whole window, the nagging behavior the plan explicitly didn't want. Documented as new AGENTS.md **Rule 81**. One-click unsubscribe via a new HMAC-signed token (`src/lib/email/unsubscribe-token.ts`, keyed on the already-required `AUTH_SECRET`, no new secret), `/api/email/unsubscribe` is intentionally public (an email-client click has no session), added to `tests/api-auth-coverage.test.ts`'s `EXEMPT` with a verified reason; `isCronAuthorized(` added to that same test's `AUTH_MARKERS` as a real non-session gate. Migration `0002_married_lockheed.sql` applied to local dev DB. **Verified**: `tsc`/lint clean, **75/75 tests** (was 70), full `npm run build` succeeds, the unsubscribe-token sign/verify round trip (including tamper + garbage rejection) and all three cron routes' query + trigger-condition logic hand-verified against real seeded progress data via a scratch script, live 401/401/200 response-code check confirming `/api/account/email-prefs` and `/api/cron/streak-reminder` correctly reject unauthenticated requests while `/api/email/unsubscribe` correctly serves its public "invalid link" page. **Not verified**: an actual cron-triggered send reaching a real inbox, needs `CRON_SECRET` set plus Vercel's scheduler actually invoking the route on the live deployment, plus #26's real email credentials, none available in this environment. This closes out every IMPROVEMENT_PLAN Tier-1 item (#23-#28, #30) except the deliberately-deferred #22 interlink map, which the owner is resuming personally. Docs updated: README (key files, env vars, `.env.example`, Settings description, test count), AGENTS.md Rule 81, IMPROVEMENT_PLAN.md status log. 1 commit on `development-branch`, not pushed.)
> Last audited: 2026-09-17 (Session 88 cont., **IMPROVEMENT_PLAN.md Tier 1 executed: Profile cluster, Gmail-SMTP auth, and a new Superadmin dashboard + user management.** Owner: "start working on the plan, leave interlink map for later," then separately "Create superadmin dashboard, user management... add these in the plan too." **#24/#25/#27 Profile cluster (done + verified live)**: new `src/lib/profile.ts` (`ma_profile`) and `src/lib/profile-stats.ts` (`getProfileStats()`, one aggregator replacing 4x duplicated stat derivations across Achievements/SkillMap/Portfolio/certificates), new `<StatsRow>`/`<ActivityHeatmap>` (18-week heatmap fed by `xpByDay`, existed since the XP system shipped but was never rendered anywhere), `/profile` hub aggregates identity/XP/streak/badges/certificates/portfolio/bookmarks/review-due, all linking to their existing pages, guest + signed-in, `noindex`. `OnboardingModal` now optionally captures a name (rides `cert-name.ts`) + stores the picked goal as `profile.primaryGoal`. Settings gained 4 cards (Profile Details, Account, Preferences, Autosave, new shared `<AutosaveIndicator>`). Nav got a persistent Profile entry point. **#26 Gmail-SMTP magic-link auth (code-complete, not yet live-verified)**: `emailAuthConfigured()` (`AUTH_SECRET`+`EMAIL_SERVER`+`EMAIL_FROM`) now drives every sign-in entry point; `nodemailer@^7` (pinned to satisfy `@auth/core`'s peer range) + a new `src/lib/email/` (branded table-based HTML/text layout, `magic-link`/`welcome`/`account-deleted` templates) wired into a NextAuth `Nodemailer` provider with a custom `sendVerificationRequest`; `/login` is now a single email-input form; Google OAuth stays wired but every former `signIn("google")` call site now points at `/login` instead. Zero DB schema change needed for this part (`verificationTokens` already existed). **#30 Superadmin dashboard + user management (new, owner-requested mid-session, built + verified this session)**: a `SUPERADMIN_EMAILS` env var, deliberately never persisted to the database anywhere, checked fresh on every request via `isSuperAdminEmail()`, so it can't be escalated from inside the app unlike the existing DB-persisted `role`/`ADMIN_EMAILS` tier (Rule 78). New `users.suspended` column + new append-only `adminAuditLog` table (migration `0001_overrated_dagger.sql`, applied to local dev DB and verified via a scratch script exercising every query against the real SQLite dialect, stats aggregation, search, audit insert/read all correct). Suspension is enforced in three independent places (signIn callback denies new sign-in, session payload carries `isSuspended` for defense-in-depth on an already-active session, and the 3 routes that call `auth()` directly instead of `requireUser()` each check it themselves), documented as AGENTS.md **Rule 80**, since missing any one of the three leaves a partial access hole. `/admin` (any admin, read-only stats) + `/admin/users` (superadmin-only: search/promote/demote/suspend/delete, guarded so a superadmin can never act on another superadmin or on themselves) + the audit log UI. `tests/api-auth-coverage.test.ts`'s `AUTH_MARKERS` updated to include `requireSuperAdmin` (2 routes failed until this was added). **Verified across all three pieces**: `tsc --noEmit` clean, `eslint` clean (one pre-existing unrelated warning), **70/70 tests** (was 66; +4 from the auth-coverage marker fix and prior quiz work), full `npm run build` succeeds (confirms nodemailer bundles fine, no edge-runtime conflicts), `/admin`+`/admin/users` correctly redirect unauthenticated visitors to `/login`. **Not verified**: an actual real Gmail-App-Password send-and-click round trip for #26, needs the owner's real `EMAIL_SERVER`/`EMAIL_FROM` set on Vercel; a real signed-in admin/superadmin browser session for #30's UI, needs a real sign-in (blocked on the same #26 email credentials) plus a `SUPERADMIN_EMAILS` value on the deployment. Docs updated same-session: README (Routes table: `/profile`/`/admin`/`/admin/users`, updated `/login`/`/account`; Key Files table: all new lib files; env-vars table + `.env.example`: `EMAIL_SERVER`/`EMAIL_FROM`/`SUPERADMIN_EMAILS`), `IMPROVEMENT_PLAN.md` (#24/#25/#26/#27 marked done, #30 added with full design), AGENTS.md Rule 80. 8 commits total across the three pieces on `development-branch`, not pushed (owner has not asked for a push this session).)
> Last audited: 2026-09-17 (Session 88 cont., **Multi-perspective audit + SEO/meta fixes.** Ran an 8-perspective audit (learner, SEO, performance, a11y, privacy, business, maintainer, AEO/GEO) compiled into `IMPROVEMENT_PLAN.md` (root, an execution tracker). Executed the first two P0 items: (1) **SEO canonical bug**, the root layout's `alternates.canonical: BASE` was inherited by every non-overriding route, pointing ~all glossary/hub/reference pages at the homepage (mass duplicate-content signal); removed it and added per-route self-canonicals to the homepage + 15 content pages + 3 dynamic `generateMetadata` pages. (2) **Title double-suffix**, 30 pages hardcoded `| Marketing Academy` in their document `title` while the `%s | Marketing Academy` template also appended it; stripped the suffix from document titles (left `openGraph.title` branded). Both verified in-browser, `tsc`/lint clean. New AGENTS.md **Rule 79** documents both metadata-inheritance gotchas. Remaining audit items (sitemap freshness, `llms.txt`, certificate name field, `CATEGORY_INDEX` bundle regression, structured-data gaps, PostHog consent, the 642-lesson interlink map queued LAST) tracked in `IMPROVEMENT_PLAN.md`.)
> Last audited: 2026-09-17 (Session 88, **Quiz UX: always-available "Retake quiz" + per-question review on the pass screen.** `Quiz.tsx` only; the retake button now shows on every finished screen (pass/fail/already-passed) without gating behind `!alreadyPassed`, and the full review renders on pass as well as fail. `handleRetry()` never clears the persisted pass flag, so a retake can't re-lock an earned lesson. Verified in-browser, `tsc`/lint clean. Merged to `main` via [PR #26](https://github.com/Surya8991/Marketing-Academy/pull/26) (merge commit `3b1bcad`). Docs updated: README quizzes line, AGENTS.md Rule 25, Session History row 88. See that row for detail.)
> Last audited: 2026-08-20 (Session 87 continued — **Accounts/sync final whole-branch review + fix wave + PR #23 merged to `main`.** After the 17-task build below, a final review (most capable model, per subagent-driven-development's own rule for this step) found 2 Critical + 4 Important + 5 Minor real issues, all fixed in ONE consolidated dispatch and independently re-verified (scoped re-review found 2 new residuals from the fix itself, both adjudicated and parked rather than triggering a second fix wave, per the skill's explicit "no per-finding fixers, no second fix wave" rule). Full detail in AGENTS.md Rule 78; summary: (1) the local sync timestamp is now scoped to the user id that wrote it (`{userId, at}`, not a bare number) — fixes a real cross-user/guest data-leak on shared browsers, accepted tradeoff is a first-time-sign-in-with-existing-server-row always has the server win; (2) `src/server/db/client.ts`'s `db` export AND `auth.ts`'s `DrizzleAdapter(...)` construction are both wrapped in a `lazyProxy<T>` helper — the adapter reads a property on `db` at construction time, so lazy-wrapping only the client wasn't sufficient to keep the "zero env vars, nothing changes" guarantee true; (3) new `src/lib/session-cookie.ts` checks both `__Secure-authjs.session-token` and `authjs.session-token` instead of guessing from `NODE_ENV`; (4) `auth.ts`'s `session()` callback now returns an explicit allow-list (`{expires, user:{id,name,email,image,isAdmin}}`) instead of spreading the adapter row, closing a `sessionToken`-leak into client-readable JSON — side effect: `role` no longer reaches `session.user`, which `api/account/delete`'s admin-self-delete guard still reads, narrowing it to `ADMIN_EMAILS`-only (parked, low impact, no admin UI exists to diverge role from ADMIN_EMAILS today); (5) new `src/lib/quiz-keys.ts` holds the three quiz-key-prefix constants moved out of the 2.4MB `quizzes.ts`, since `progress-snapshot.ts` needs them and is now in the client bundle via `SyncProvider` (Rule 41 bundle-size precedent); Nav's sign-in button and a new mobile-menu entry point are both gated behind a real `authConfigured` prop (previously ungated, and mobile had no sign-in entry at all). Also parked/disclosed, not fixed: `restoreAllKeys()`'s additive-only semantics can still leave residual keys from a prior user visible to the next signed-in user on a shared browser in some flows — needs a real product decision, not a quick patch. Final state: **65/65 tests**, `tsc`/lint/build all clean, merged via [PR #23](https://github.com/Surya8991/Marketing-Academy/pull/23) to `main`. `README.md` was updated in the same fix-wave commit (401 vs 503, test count); this entry plus AGENTS.md Rule 78 bring `PROJECT_LOG.md`/`AGENTS.md` current with the final shipped state, since the Task 17 docs commit below predated these fixes. Also shipped: a polished HTML handoff artifact (published, not committed to the repo) listing the 3 manual/human-only follow-ups — Google OAuth client + redirect URIs, Turso database provisioning for production, and a two-browser manual verification walkthrough — since none of those are things a build step can do.)
> Last audited: 2026-08-20 (Session 87 — **Accounts + sync feature shipped end-to-end (17 tasks, this session).** Real Google sign-in via NextAuth v5 (`src/auth.ts`, `DrizzleAdapter`), backed by a new SQLite/Turso database (`src/server/db/` — `schema.ts` with `users`/`accounts`/`sessions`/`verificationTokens`, `client.ts`, `migrations/`). Every var is optional and the whole feature no-ops cleanly when unset (`authConfigured()` in `src/lib/env.ts` gates whether sign-in UI even renders). New routes: `/login` (`src/app/login/page.tsx` + `SignInButton.tsx`), `/account` (`src/app/account/page.tsx` + `AccountClient.tsx` — profile, session list, delete-account), `/api/auth/[...nextauth]`, `/api/account/sessions`, `/api/account/delete`, `/api/sync`. **`/api/sync-proxy` and its `SYNC_SECRET`/`NEXT_PUBLIC_SYNC_SECRET` shared-secret design are DELETED** (AGENTS.md Rule 26/44, both annotated resolved, not rewritten, per this file's own established convention) — replaced by `/api/sync`, gated by `requireUser()`'s real per-user database session, reading/writing a per-user row instead of one shared global KV key. Client-side auto-sync (`src/lib/sync-client.ts`: `mergeSnapshots`/`pullAndMerge`/`pushNow`/`startAutoSync`) debounce-pushes on a new `PROGRESS_CHANGED_EVENT` (`src/lib/events.ts`), which every localStorage progress-writer now dispatches — see AGENTS.md Rule 77 for the full pattern and which functions dispatch it. `src/lib/progress-snapshot.ts` is new: the single shared definition of "what counts as the user's progress data" (`EXPORT_KEYS`/`ALLOWED_KEY_PREFIXES`/`collectAllKeys`/`restoreAllKeys`), consumed by both `/settings` export/import/reset and the new sync feature, so the two can't drift into different definitions (Rule 18's discipline, one level up). Admin access (`isAdminUser`/`requireAdmin` in `auth.ts`) uses a persisted `users.role` column bootstrapped from a new `ADMIN_EMAILS` env var, mirroring the Email-Automator sister project's pattern exactly. New env vars, all optional, documented in `.env.example` and README.md: `AUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `DATABASE_URL`, `TURSO_AUTH_TOKEN`, `ADMIN_EMAILS`. Nav.tsx gained sign-in UI. New test file `tests/api-auth-coverage.test.ts`. `npm test`, `tsc --noEmit`, `npm run lint`, `npm run build` all clean at every task boundary. This entry is Task 17 of the plan, docs-only, per AGENTS.md Rule 23.)
> Last audited: 2026-08-19 (Session 86 continued - **🎉 STAGE 9.3'S NARROWED 8-CATEGORY SCOPE IS NOW FULLY COMPLETE.** After `paid-ads` (below) and the Nav mega-menu fix were committed and pushed, the owner asked to continue through the remaining 7 categories (`seo`, `product-marketing`, `growth`, `social`, `ai-marketing`, `fundamentals`, `tools`) with a single push at the end, not one per category. Executed as ~85 two-lesson `general-purpose` agent batches, merged category-by-category with `merge-projects-batch.mjs`, each category verified with the full `tsc --noEmit` → `npm run lint` → `npm test` → `npm run build` chain before moving to the next. **Real gotcha found, documented as AGENTS.md Rule 76**: `scripts/get-category-batch-info.mjs` doesn't resolve `sourceCategory`, so it overcounted `fundamentals`' remaining lessons at 32 when the true count was 19 — 13 were the cross-listed `mental-models` lessons (Rule 31/43) already covered under `mental-models.ts`, correctly skipped rather than double-authored under the wrong category. Recurring mechanical defects, all caught only by `tsc`/`npm test`, none by the audit script: duplicate `stepId` at both the start and end of a `ProjectStep` object (`growth.ts`, `tools.ts`); `paidUpgradeNote` nested inside a `ToolRef` instead of as a `ToolStack` sibling (`ai-marketing.ts`, twice); a non-exact `toolName` string failing the audit's exact-match check (`"TikTok Ads Manager"` should have been `"TikTok Ads"`). Two batches (`ai-marketing`'s H and N) self-merged and ran their own full verify chain rather than staying in scratch as instructed — harmless here since the merge script refuses duplicate keys, but required checking `git status`-equivalent state before merging the rest of that category's batches to avoid double-merging. 11 more real, previously-uncataloged tools added to `tools-directory.ts` (Rule 55): TikTok Shop for Business, Meta Commerce Manager, TINT, Bazaarvoice, HypeAuditor, Grin, Supermetrics, Google Ads Editor, SpyFu, Cursor, plus PartnerStack from the `paid-ads` merge — `tools-directory.ts` grew from 141 to **157**. Mid-session, the owner also shared a reference page (Edstellar's own trainers directory) for the Topics mega menu's visual pattern; redesigned it a second time from a flat responsive grid to a two-pane flyout (left sidebar of discipline tabs, right panel showing that group's topics), verified via Playwright across breakpoints and both themes. Final verified state: **803 project cards across 19 category modules** (up from 477 at the very start of Session 86). The only item left anywhere in the original 402-lesson Stage 9.3 backlog is `events-experiential`'s remaining 20 lessons, explicitly out of scope per the owner's category-narrowing directive from Session 85. Commits remain UNPUSHED per the owner's explicit instruction to push once everything in this batch is done.)
> Last audited: 2026-08-19 (Session 86 - **Two independent pieces of work.** (1) **Stage 9.3, `paid-ads` category now fully done, 33/33 lessons.** The 18 remaining lessons each got 2 practice projects + 2 `InAction` concept scenarios, authored via 9 parallel 2-lesson `general-purpose` agent batches (A-I) following `PROJECTS_AUTHORING_GUIDE.md`. Added 5 real platform tools to `tools-directory.ts` first, since this category genuinely needed them and none existed yet (Amazon Ads, Apple Search Ads, Pinterest Ads Manager, Snapchat Ads Manager, Reddit Ads Manager — Rule 55). Merged via `merge-projects-batch.mjs`; two agents wrote non-exact `toolName` strings that failed the audit script's exact-match check ("Google Ads (Demand Gen campaign type)", "Google Ads (AI Max / broad match)" — fixed to plain "Google Ads"), and one agent left a duplicate `mode: "teardown"` object-literal property, a `tsc` hard error the audit script doesn't check (Rule 68's recurring pattern). Audit, `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean after fixes; live-checked one lesson page and one project page in a fresh browser tab. Index regenerated: **529 project cards, 19 category modules.** 7 of the 8 in-scope Stage 9.3 categories remain (`fundamentals`, `seo`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools`). (2) **Nav.tsx Topics mega menu redesigned**, owner-reported "not looking good." Root cause: the panel used CSS `columns-3` masonry (uneven per-column heights, awkward group-splitting) AND was anchored (`absolute left-0`) to the small Topics *button* rather than the header, so on a 1440px-wide viewport the 1040px-wide panel overflowed off the right edge of the screen entirely — the 5th column (`Career & Legal`) was clipped outside the viewport, not just visually crowded. Fixed by converting to an explicit responsive CSS grid (`grid-cols-2 sm:grid-cols-3 lg:grid-cols-5` with `divide-x`/`divide-y`) with a discipline icon per group, and by anchoring the panel to the full header container (`left-4 sm:left-6 lg:left-8`, width capped at `min(1040px, calc(100vw-2rem))`) instead of the button, so it can never overflow the viewport regardless of where the trigger sits. Verified via a temporary local Playwright install (not a project dependency) at 1440/1024/768px in both light and dark mode — no horizontal scroll, responsive column collapse works, dark-mode CSS variables render correctly. `tsc --noEmit` and `eslint` both clean.)
> Last audited: 2026-08-18 (Session 85 continued - **Branch cleanup, owner directive: clean up branches before starting Stage 9.3.** Repo down to exactly 2 branches (local + remote): `main` and `stage-8-3a-paid-ads-mastery`. Deleted 4 stale branches after checking each against `origin/main` (`git merge-base --is-ancestor`): `feat/stage-8-projects-pilot`, `fix/stage-0-p0-safety-fixes`, `fix/stages-3-6-7-remaining` were fully merged (zero content loss, safe delete); `improvements/review-queue-and-audit-fixes` (forked 2026-08-05, 72 commits stale, unmergeable as-is) contained real still-open issues, checked one by one against current code before deleting: **re-derived fresh** the still-live XP-farming bug (`TrackQuizPageClient.markAll()` re-awarded XP for lessons completed >24h ago on quiz retake, since `addXP()`'s own dedup is only a rolling 24h window — this is literally the last item in AGENTS.md Rule 36's "five interlocking integrity defects" list), 2 stale hardcoded-count strings (`/tracks` metadata "22...8...14", `/interview-prep` "257 lessons"), tracks never being searchable anywhere (added a `track` type to `commandIndex.ts`/`CommandPalette.tsx` and a discriminated union to `/search`'s `SearchClient.tsx`), and `prefers-reduced-motion` support (was completely absent site-wide, one additive `@media` block in `globals.css`). **Deliberately NOT re-derived** (bigger net-new feature work than a fix, flagged as a future backlog item instead of building unprompted): the branch's spaced-repetition `/review` page + `reviewQueue.ts`, a per-lesson print button, and a "rebuild tracks 24→30" content change that conflicts with the 24-track count now deeply embedded across current docs/tests. Local `main` was reset to `origin/main` (was 26 behind / 1 stale duplicate ahead — that 1 commit's content already existed upstream under a different hash from the PR #16 merge). `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean. Per "push once everything is done only," all commits remain UNPUSHED — **NEXT UP: Stage 9.3** resumes with its narrowed 8-category scope.)
> Last audited: 2026-08-18 (Session 85 continued - **STAGE 11 COMPLETE.** Owner-requested UX pass on `/skill-map`, `/achievements`, `/resources`: all 3 had predated the 2026-08-12 field-manual type-system redesign (`font-display`/`font-ui-sans`/`font-data`, `PageMasthead`) that every other major page already had. Skill Map: PageMasthead, specimen stats row, category cards regrouped by discipline reusing the same `TOPIC_GROUPS` taxonomy just added to Nav.tsx (extracted to new `src/lib/topic-groups.ts` so the two can't drift apart), fixed a stale "15 disciplines" string (real count 21). Achievements: PageMasthead, specimen stats row, 12 badges regrouped into 4 labeled sections instead of one flat grid. Resources: PageMasthead + type classes on hero/section headings. `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean. **Owner then asked to pause before Stage 9.3 and do branch cleanup + any other pending work first** — pivoting to that next. Per "push once everything is done only," these commits remain UNPUSHED.)
> Last audited: 2026-08-18 (Session 85 continued - **STAGE 10 FULLY COMPLETE (10.1-10.4).** Every one of 642 lessons now has 5 quiz questions (was 4): 642 net-new questions authored across 21 category batches using a new `scripts/merge-quiz-questions.mjs` (bracket-depth-matched insertion into the single large `quizzes.ts` file, backs up first, refuses to double-insert). `quizzes.ts` grew to ~2.4MB (3,210 total questions, 12,840 option strings). `Quiz.tsx`'s `PASS_THRESHOLD` updated 0.75→0.8 (4 of 5 correct, same forgiveness as the old 3 of 4); `TrackQuizPageClient.tsx` already used 0.8, no change needed. Stale "4 questions"/"4/4" references fixed in AGENTS.md (Rule 25, Rule 41) and README.md (features list, Key Files table). Re-ran the position-dependent-option ("all/none of the above") scan across the full expanded option set: 0 matches. Also this session: desktop Nav.tsx Topics dropdown rewritten to group categories by discipline (Strategy/Channels/Growth & Data/Outreach/Career & Legal), reusing the mobile menu's existing `TOPIC_GROUPS` taxonomy instead of a flat list, per owner screenshot feedback. Unpushed quiz-authoring commits squashed twice via `git reset --soft` to keep the branch's history readable. `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean throughout. **NEXT UP: Stage 11** (owner-requested Skill Map/Achievements/Resources UX audit+improvements — scope TBD, audit first), then Stage 9.3 resumes last with its narrowed 8-category scope. Per explicit owner instruction ("Push once everything is done only"), these commits remain UNPUSHED, deliberately, until told everything is done — do not push without being asked.)
> Last audited: 2026-08-18 (Session 85 continued - **Stage 9.3 scope restricted by owner directive (second reorder).** Owner reviewed the full category list and scoped 9.3 down to 8 categories only: `fundamentals`, `seo`, `paid-ads`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools`. Everything else that still has non-track lessons without projects — `content`, `email`, `analytics`, `copywriting`, `cro`, `events-experiential` (20 lessons left after the 8 already done), `affiliate-marketing`, `marketing-leadership`, `legal-compliance` — is now explicitly OUT OF SCOPE for 9.3 until told otherwise, not merely deprioritized. This does not affect Stage 10 (quiz expansion), which still covers every category. **Also this session: PR #16 opened and merged** (`stage-8-3a-paid-ads-mastery` → `main`, 25 commits: Stage 8.3a completion, Stage 9.1 portfolio, Stage 9.3 events-experiential partial, Stage 9.4 hygiene, Stage 10.1 quiz batches through `growth`, and the Vercel image-quota fix) — confirmed merged via `gh pr view`. The feature branch was kept (not deleted) since work continues on it; future pushes will need another PR merge to reach `main`. `gh auth login` as `Surya8991` (done earlier this session) resolved the previous 403 push blocker for good.)
> Last audited: 2026-08-18 (Session 85 continued - **Stage 9.3 started, then paused per owner directive.** Owner asked to move 9.3 (the 402-lesson, ~800-project non-track backlog) to run LAST, after Stage 10 and a new owner-requested Stage 11 (Skill Map/Achievements/Resources UX pass), since 9.3 alone is estimated at 10+ hours of continuous execution. One category was already in flight when the directive landed (`events-experiential`, 8/28 lessons, 4 parallel batches) — finished and committed rather than discarded rather than wasting the work, then 9.3 paused there. New script: `scripts/get-category-batch-info.mjs` (sibling to get-track-batch-info.mjs, works from curriculum.ts directly for lessons with no track). Added `Eventbrite` to `tools-directory.ts` (no event-registration tool existed). One `conceptsCovered` omission on a teardown project caught by `tsc --noEmit`, fixed. `audit-projects.mjs`, lint, `npm test` (30/30), `npm run build` all clean. Index regenerated: 493 project cards, 19 category modules. **NEXT UP: Stage 10** (expand every quiz 4→5 questions), then Stage 11 (Skill Map/Achievements/Resources), then 9.3 resumes last. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - **Stage 9.1 (career layer) partially shipped.** New `/portfolio` page (`src/app/portfolio/page.tsx` + `src/components/PortfolioClient.tsx`): lists the learner's completed practice projects as portfolio-ready evidence (company/tier/archetype/mode/concepts), reads `projects-progress.ts` localStorage matched against `PROJECTS_INDEX` (slim client bundle, Rule 41-safe), "Export as JSON" via client-side Blob download, cross-linked both ways with `/interview-prep`. Wired into `Nav.tsx`'s "Your Progress" section and `Footer.tsx`. Two new achievements (`first-project`, `ten-projects`) added to `achievements.ts`, reusing the existing `checkAchievements()` call already present in `ProjectCard.tsx`'s completion handler — no new wiring needed there. `robots: noindex` on the page (personal, per-browser data, same treatment as `/achievements`). **Not done, correctly out of scope**: the `calibration` mode's deferred-verification infrastructure (prediction persistence, 30/90/180-day re-prompting, personal prediction log) and big-projects-feeding-`/certificates` — both are real, separate Phase-3 infra work, not a quick add. `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - **Stage 9 begun.** A dedicated research agent audited the real current state of Stages 0-7, 8.2b, and all of Stage 9's items against actual code (not stale doc claims): Stages 0-7 and 8.2b confirmed genuinely done (stage-numbered comments throughout the code). Stage 9.1 (`/portfolio` + `calibration` deferred-verification infra) confirmed not started, correctly scoped as Phase 3. Stage 9.2 (Related Concepts): real count is 74/642 lessons (11.5%), not the doc's stale 63/642 (9.8%) — 568 lessons still need it. Stage 9.3 (non-track lessons with zero projects): confirmed still exactly 402 (642 total − 240 with ≥1 project); also found 4 entire categories with zero project files (`events-experiential`, `affiliate-marketing`, `marketing-leadership`, `legal-compliance`) and that `track-projects.ts` (track-level big projects) only covers 1 of 24 tracks. Stage 9.4 (hygiene) executed and closed this session: BOMs already 0, single-quoted `lessonMeta` fixed in all 25 files (74 strings converted via a scoped script touching only the `lessonMeta` object, none needed escaping), bloated lesson left untouched per the plan's own low-priority guidance. `tsc --noEmit`, lint, `npm test` (30/30), `npm run build` all clean. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #23 track: `pr-communications-mastery` rollout, 8 remaining lessons (6 already had projects from an earlier session). 4 parallel agents (batches A-D, 2 lessons each). Two agents (B, D) stalled silently mid-run with no completion notification — diagnosed via scratch-file/MDX timestamp checks, relaunched scoped to only the missing work to avoid duplicating already-landed InAction inserts. `tsc --noEmit` caught a real defect batch A introduced (duplicate `mode` property on 2 projects) — fixed. `audit-projects.mjs`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. Regenerated `/projects` hub index (477 total project cards, 18 category modules). `pr-communications-mastery` track is now fully done, 14/14. **THIS WAS THE FINAL TRACK — all 24 tracks in Stage 8.3a's locked priority order are now complete.** Proceeding to Stage 9 per the standing instruction. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #22 track: `psychology-of-marketing` rollout, all 14 lessons, first projects in this category (`src/lib/projects/psychology.ts` created new, `PSYCHOLOGY_PROJECTS` export). 7 parallel agents (batches A-G, 2 lessons each). Merge, `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass — no Rule 67-71 defects recurred. Regenerated `/projects` hub index (461 total project cards, 18 category modules). `psychology-of-marketing` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #21 track: `brand-strategy-mastery` rollout, all 14 lessons, first projects in this category (`src/lib/projects/brand-strategy.ts` created new, `BRAND_STRATEGY_PROJECTS` export). 7 parallel agents. `audit-projects.mjs` caught one real defect: a simulation stage's `lessonAnchor` pointed at bolded prose text instead of a real heading — fixed to the real containing `## Common Mistakes` heading; also fixed two `lessonRef` values that used a raw slug instead of the codebase's established descriptive-prose convention. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. Regenerated `/projects` hub index (433 total project cards, 17 category modules). `brand-strategy-mastery` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #20 track: `product-marketing-mastery` rollout, 9 remaining lessons, none with `InAction` scenarios. 5 parallel agents, merged in one call. `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (405 total project cards, 16 category modules). `product-marketing-mastery` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #19 track: `social-media-manager` rollout, 10 remaining lessons across 3 categories (`social` x8, `copywriting` x1, `tools` x1), none with `InAction` scenarios. 6 parallel agents, merged per category. `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (387 total project cards, 16 category modules). `social-media-manager` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #18 track: `ecommerce-growth` rollout, only 1 remaining lesson (`reengagement`, 17/18 already covered via category overlap from earlier tracks this session). Single agent, merged into `email.ts` (17 lessons total). `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (367 total project cards, 16 category modules). `ecommerce-growth` track is now fully done, 18/18. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #17 track: `growth-marketing-mastery` rollout, 11 remaining lessons, none with `InAction` scenarios. 6 parallel agents, merged in one call. `audit-projects.mjs` caught the same `mode: "teardown"` but `steps[]`-populated defect class from an earlier session batch on 2 projects — both fixed to `mode: "diagnostic"`. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. Regenerated `/projects` hub index (365 total project cards, 16 category modules). `growth-marketing-mastery` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #16 track: `content-strategy` rollout, 8 remaining lessons across 2 categories (`content` x7, `seo` x1), none with `InAction` scenarios. 5 parallel agents, merged per category. `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (343 total project cards, 16 category modules). `content-strategy` track is now fully done, 14/14. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #15 track: `copywriting-mastery` rollout, 11 remaining lessons (`features-vs-benefits`, `power-words`, `voice-and-tone`, `landing-page-copy`, `cta-copy`, `email-copy`, `value-prop-copy`, `storybrand`, `b2b-copy`, `copy-for-ai-search`, `ogilvy-halbert-sugarman`), none with `InAction` scenarios. 6 parallel agents, merged in one call. `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (327 total project cards, 16 category modules). `copywriting-mastery` track is now fully done, 15/15. STILL UNRESOLVED: the Vercel image-quota fix on `main` (`e78b342`) remains unpushed, see the line below for details, push access is still needed.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #14 track: `mental-models` rollout, 5 remaining lessons (`pattern-recognition`, `inversion-thinking`, `systems-thinking`, `pareto-and-constraints`, `decision-making-under-uncertainty`), none with `InAction` scenarios. `pattern-recognition` is one of the 13 cross-listed lessons (Rule 31/43) but needed no special handling since its MDX and project key both live under `mental-models` canonically. 3 parallel agents, merged in one call. `audit-projects.mjs`, `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean on the first pass. Regenerated `/projects` hub index (305 total project cards, 16 category modules). `mental-models` track is now fully done, 13/13. Also, mid-session, fixed an urgent unrelated issue at the owner's request: Vercel's free-plan Image Optimization Transformations quota (5,000/month) was exceeded by the site's only `next/image` usage, YouTube thumbnails in `LessonResourcesClient.tsx`. Set `images.unoptimized: true` in `next.config.ts` so `next/image` serves originals directly instead of routing through Vercel's paid optimizer. Verified clean on `main` and committed there directly (`e78b342`, cherry-picked from this session's branch) since it needed to ship independent of the larger in-progress content branch — but push to `origin/main` failed with a 403, the environment's authenticated git identity (`edstellarmarketing`) lacks write access to `Surya8991/Marketing-Academy`, and a stored `Surya8991` GitHub CLI token was invalid/expired with no way to re-authenticate non-interactively. The fix is committed locally on `main` but NOT yet on GitHub/deployed, the owner needs to push it themselves or restore push access.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #13 track: `content-creator` rollout, 5 remaining lessons across 4 categories (`mental-models`, `copywriting`, `content`, `tools`). `repurposing`'s second cappedTierPair slot was `core-feeds-big` (a track-level big-project synthesis input, not a per-lesson project per `compute-project-assignment.mjs`'s own docs), so only 1 project was authored for it rather than 2. 4 parallel agents, one per category. `audit-projects.mjs` clean on all 4 categories with no fixes needed this time (the Rule 71 slug-algorithm gotcha from the prior batch didn't recur). `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean. Regenerated `/projects` hub index (295 total project cards, 16 category modules). `content-creator` track is now fully done, 18/18.)
> Last audited: 2026-08-18 (Session 85 continued - confirmed `solo-founder` already fully done, 18/18, via category overlap from other tracks worked this session, no new authoring needed. Then executed PROJECTS_PLAN.md Priority #12 track: `freelancer-agency` rollout, only 2 remaining lessons across 2 categories, `instagram` (`social`) and `all-in-one-tools` (`tools`) — first projects in BOTH categories, created stub files `src/lib/projects/social.ts`/`tools.ts` before merging. `audit-projects.mjs` caught 2 anchor-slug mismatches rooted in the site's own slug algorithm being a simplified approximation of `rehype-slug`/`github-slugger`, not the real library (verified by reading `tests/projects-data.test.ts`'s own regex rather than guessing): a slash-containing heading collapses runs of removed-then-whitespace characters to a SINGLE hyphen on this site, not the double hyphen real `github-slugger` would produce. Both fixed. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean. Regenerated `/projects` hub index (286 total project cards, 16 category modules). `freelancer-agency` track is now fully done, 16/16.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #10 track: `email-lifecycle-mastery` rollout across all 12 remaining lessons in the `email` category. None had `InAction` scenarios. Ran `get-track-batch-info.mjs email-lifecycle-mastery` for 6 batches, launched 6 parallel `general-purpose` agents. Merged via one `merge-projects-batch.mjs email` call; the merge reported 11 keys but a 12th (`segmentation`) had been written unquoted (Rule 58) and its content still landed in the file (the script pastes whole batch bodies, not key-by-key), caught by a `grep` after the count looked short and fixed by quoting. Three further defects caught only by `tsc`/`npm test`, none by the audit script: a `paidUpgradeNote` field misplaced inside a `ToolRef` instead of `ToolStack`; two more instances of `archetype: "build"` (a `ProjectMode` value, not a real `Archetype`, same class as the CRO/Data-Driven Marketer batches' `"calibration"` mistake) fixed to `"build-the-asset"`; and a `mode: "simulation"` project with a completely empty `stages[]` (the agent wrote simulation-flavored prose but never built real stage objects), fixed by converting to `mode: "diagnostic"` with 2 real `ProjectStep`s built from its own existing content. `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after fixes. Regenerated `/projects` hub index (282 total project cards). `email-lifecycle-mastery` track is now fully done, 15/15.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #9 track: `analytics-mastery` rollout. Only 3 of its 15 lessons needed anything (`cohort-analysis`, `attribution-models`, `privacy-sandbox`), the other 12 already had projects from the Data-Driven Marketer batch's category overlap earlier this session. First launch of the 2 batches was interrupted by a harness process restart mid-run with zero output ever written to disk (confirmed by checking the scratch directory and grepping all 3 MDX files for `InAction` before relaunching, rather than assuming either agent's work landed) — relaunched both clean. Merged via one `merge-projects-batch.mjs analytics` call (3 keys, 6 projects, 16 lessons total in the file). `audit-projects.mjs` clean; `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean. Regenerated `/projects` hub index (258 total project cards). `analytics-mastery` track is now fully done, 15/15.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #8 track: `cro-mastery` rollout across all 15 lessons in the `cro` category, the FIRST practice projects in this category (`src/lib/projects/cro.ts` created new this batch, `CRO_PROJECTS` export). None had `InAction` scenarios either. Ran `get-track-batch-info.mjs cro-mastery` for 8 batches (7x2 + 1x1), launched 8 parallel `general-purpose` agents, merged in one `merge-projects-batch.mjs cro` call (15 keys, 30 projects). 30 new practice projects + 30 `InAction` scenarios authored. `audit-projects.mjs` caught one real defect: `friction-audit-session-recording-teardown` had `mode: "teardown"` but populated `steps[]` not `teardownItems[]` — fixed by correcting `mode` to `"diagnostic"` (its `archetype: "teardown"` was already right). `tsc --noEmit`, `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. Regenerated `/projects` hub index (252 total project cards). `cro-mastery` track is now fully done, 15/15.)
> Last audited: 2026-08-18 (Session 85 continued - executed PROJECTS_PLAN.md Priority #7 track: `data-driven-marketer` rollout across all 13 remaining lessons spanning 3 categories (`mental-models` x2, `analytics` x10, `growth` x1; the other 6 lessons already had projects from earlier sessions). None of the 13 had `InAction` scenarios yet, so every batch authored both. Ran `get-track-batch-info.mjs data-driven-marketer` for 7 batches, launched 7 parallel `general-purpose` agents, merged per category (mental-models 1 file/2 keys, analytics 5 files/10 keys, growth 1 file/1 key) via `merge-projects-batch.mjs`. 26 new practice projects + 26 `InAction` scenarios authored. `audit-projects.mjs` clean on all 3 categories; `tsc --noEmit` caught a real defect the audit script missed: two agents wrote `archetype: "calibration"` (a valid `ProjectMode` value but not a real `Archetype`) — fixed to `"reverse-engineer"` on both. `npm run lint`, `npm test` (30/30), `npm run build` all clean after the fix. Regenerated `/projects` hub index (222 total project cards). `data-driven-marketer` track is now fully done, 19/19.)
> Last audited: 2026-08-18 (Session 85 - executed PROJECTS_PLAN.md Priority #6 track: `paid-ads-mastery` rollout across all 14 remaining lessons in the `paid-ads` category (single-category track). 12 of the 14 lessons already had `InAction` concept scenarios from earlier uncommitted work at session start; only `ios-attribution` and `ad-fraud-invalid-traffic` needed scenarios authored fresh alongside projects. Ran `get-track-batch-info.mjs paid-ads-mastery` for 7 batches of 2 lessons, launched 7 parallel `general-purpose` agents per `PROJECTS_AUTHORING_GUIDE.md`. Authored 28 new practice projects (leaning heavily on `simulation` mode since real paid-ads practice costs real ad spend, mixed with `diagnostic`/`audit`/`teardown`/`build` where a free supplied-artifact angle existed) + 4 new `InAction` scenarios (ios-attribution, ad-fraud-invalid-traffic). Merged via `merge-projects-batch.mjs` into `src/lib/projects/paid-ads.ts` (now 15/15 lessons, 30 projects total in that file). `audit-projects.mjs` and `tsc` clean; `npm test` initially caught one real defect neither of those checks would have (several simulation stages used `nextStageId: ""` for a terminal option instead of the `"end"` sentinel the test suite requires) — fixed with a targeted find-replace, re-verified 30/30 tests, lint, and full `npm run build` all clean. Regenerated `/projects` hub index (196 total project cards across the library). `paid-ads-mastery` track is now fully done, 15/15.)
> Last audited: 2026-08-16 (Session 84 - executed PROJECTS_PLAN.md Priority #5 track: `b2b-marketer` rollout across all 16 remaining lessons spanning 6 categories (`fundamentals`, `mental-models`, `product-marketing`, `content`, `email`, `analytics`). Authored 32 new practice projects + 32 `InAction` concept scenarios. Created `src/lib/projects/product-marketing.ts` (first projects in that category). Merged scratch files A-I into `fundamentals.ts`, `mental-models.ts`, `product-marketing.ts`, `content.ts`, `email.ts`, and `analytics.ts`. Added `Miro`, `Zoom`, and `YouTube` to `tools-directory.ts`. All 30 tests in test suite, tsc, audit-projects clean. Regenerated `/projects` hub index (168 total project cards across library).)
> Last audited: 2026-08-16 (Session 83 - executed PROJECTS_PLAN.md Priority #3 track: `off-page-seo-mastery` rollout across all 10 remaining lessons spanning `seo`, `pr-communications`, and `content`. Authored 20 new practice projects + 20 `InAction` concept scenarios. Created `src/lib/projects/pr-communications.ts` (first projects in that category). Merged scratch files A-F into `seo.ts`, `pr-communications.ts`, and `content.ts`. Added `Google Business Profile`, `Reddit`, `GummySearch`, `Brand24`, `BuzzStream`, `Muck Rack`, `Typeform`, `Google Forms`, `SurveyMonkey`, and `Datawrapper` to `tools-directory.ts`. All 30 tests in test suite, tsc, audit-projects clean. Regenerated `/projects` hub index (136 total project cards across library).)
> Last audited: 2026-08-16 (Session 82 - executed PROJECTS_PLAN.md Priority #1 track: `ai-first-marketer` rollout across all 9 remaining lessons spanning `ai-marketing`, `copywriting`, and `analytics`. Authored 18 new practice projects + 18 `InAction` concept scenarios. Merged scratch files A-F into `src/lib/projects/ai-marketing.ts`, `src/lib/projects/copywriting.ts`, and `src/lib/projects/analytics.ts`. Added `SparkToro` to `tools-directory.ts`. Wired persistent step completion in `ProjectStep.tsx` to `projects-progress.ts` (`ma_projects_progress`). Synced `seo/zero-click-search` quiz text option in `quizzes.ts` (~68%). All 30 tests in test suite, tsc, audit-projects, and build clean. Regenerated `/projects` hub index (116 total project cards across library). Scheduled 5-hour reminder timer task.)
> Last audited: 2026-08-13 (Session 81 - continuation of Session 80's project-content-architecture rollout. Migrated the remaining 59 of 60 projects in `src/lib/projects/seo.ts` to the 9-field "Learn vs Do" schema from Rule 63 (skills/prerequisites/terminology/keyQuestion/whatToLookFor/decision/professionalRecommendation/commonMistakes/keyTakeaway), completing that file, 60/60 projects now migrated. Execution: 6 sequential `general-purpose` agents (Agent tool, no Workflow opt-in), ~10 projects/agent, run one at a time rather than in parallel since all 6 batches write to the same file and a race would silently drop work; each agent grounded its content in the target project's own existing steps/teardownItems/answerKey data plus the matching lesson MDX for terminology, not invented facts, following the Session 80 reference project (`core-web-vitals-field-vs-lab-audit`) for tone and shape. One agent self-caught and fixed a malformed decision-option key it introduced mid-edit before its own verification pass. Final verification (my own pass, not just each agent's self-report): `grep -c keyTakeaway` = 60/60, `tsc`/lint/30 tests all clean, `scripts/audit-projects.mjs seo` reported zero structural issues, and a standalone script check confirmed all 60 `decision` blocks have exactly 4 options with exactly 1 `correct: true` each (the one invariant the existing test suite doesn't check for this new field). Full `npm run build` run as the final gate before push. The other 8 category files (53 more projects: ai-marketing, analytics, content, copywriting, email, fundamentals, growth, mental-models, paid-ads) remain on the OLD schema, not yet migrated, next in the one-category-at-a-time rollout per the owner's Session 80 decision.)
> Last audited: 2026-08-13 (Session 80 - two parts. (1) Full project audit at owner's request: `npm audit fix` + manual `next` 16.2.9->16.3.0 bump (patch, not major) closed 3 High CVEs (SSRF via rewrites, SSRF in Server Actions, middleware/proxy bypass) plus dompurify/mermaid moderate advisories, 0 vulnerabilities remaining, verified `tsc`/lint/30 tests/full build all clean on the new version. Corrected a stale claim in `PROJECTS_PLAN.md` (and the owner's own saved cross-session memory) that the 5 completion-integrity bugs from section 0.1-0.1f were still open; verified against live source they were actually fixed back in commit `4032018` and just never documented, PROJECT_LOG.md never got a session entry for that fix either. `/api/geo-audit`'s in-memory-Map rate limiter (same class as the deleted `/api/groq`'s) remains a real, undocumented-until-now gap, flagged not fixed, needs a shared store (Upstash/Vercel KV) this session had no credentials to provision. (2) Owner then supplied a full redesign spec plus a fully-worked example rewrite of `core-web-vitals-field-vs-lab-audit` and asked for the projects content architecture to be improved: less vague, step-by-step, jargon-defined-on-first-use, professional register, and made to "work in future" (i.e. a real schema, not one-off prose). Added 9 new OPTIONAL fields to `Project` (types.ts): `skills`, `prerequisites`, `terminology`, `keyQuestion`, `whatToLookFor`, `decision`, `professionalRecommendation`, `commonMistakes`, `keyTakeaway` — optional so all 112 other existing projects keep rendering unchanged until migrated, no test needed updating. Built the rendering in `ProjectCard.tsx` plus a new `DecisionBox.tsx` (multiple-choice checkpoint, answer hidden until picked, same reveal-timing pattern as `Quiz.tsx` per Rule 25). Restructured the dedicated project page's section order to match the owner's "Learn vs Do" architecture: Before You Start (prerequisites + terminology + tools, moved here from a separate bottom-of-page section) -> The Process (renamed from "Steps") -> What To Look For -> Decision checkpoint -> Professional Recommendation -> Common Mistakes -> Final Deliverable (NEW, see below) -> Success Criteria -> Key Takeaway. Found and fixed a real pre-existing bug while doing this: `project.deliverable` and `project.sampleOutput`, both non-optional fields every one of the 113 projects has always populated, were silently never rendered anywhere in `ProjectCard.tsx` for the entire lifetime of the projects feature; now shown as a "Final deliverable" section with the sample output behind a `<details>` disclosure. Migrated the one project the owner hand-rewrote (`core-web-vitals-field-vs-lab-audit` in `seo.ts`) end-to-end as the reference implementation, verified live in-browser (decision-box reveal, professional recommendation, common mistakes, key takeaway all confirmed rendering and behaving correctly) after hitting and fixing a real Rule 50/59 dev-server-staleness case mid-verification (a registered `ma-v2` service worker was serving a stale bundle even after a full server restart + fresh tab; unregistering it and clearing Cache Storage in that tab was required, confirms Rule 59's finding reproduces in local dev too, not just prod). Documented the new schema and page section order as AGENTS.md Rules 63-64. Owner chose to migrate the remaining 112 projects one category at a time, starting with `seo.ts` (60 projects, 1 done, 59 remaining) — NOT done yet, scoped as the immediate next task. (3) Owner then asked for more design distinctiveness ("don't fit all in card shape boxes"): the v1 redesign wrapped every new section in its own rounded-border box, read as a stack of identical widgets. Rewrote `ProjectCard.tsx`'s "full" body to match the site's existing "field manual" identity (Fraunces display / mono `font-data` eyebrows / hairline `border-t` dividers, no new colors or tokens) instead of inventing a new visual language: hairline-divided sections replace boxes for everything except genuinely interactive/complex pieces (`DecisionBox`, `ProjectStep`, `TeardownItemCard`, `SimulationRunner`, left alone). One deliberate signature move: Professional Recommendation and Key Takeaway both render as a Fraunces italic pull-quote, the two moments in a project worth typographic weight. `DecisionBox.tsx` also lightened to drop its own redundant card chrome now that its parent section supplies one. While verifying live, hit ANOTHER instance of the Rule 59 service-worker staleness bug, this time also reproducing in the owner's own browser mid-session (a stale `sparkles.mjs` chunk reference after the rewrite removed that icon import) — root-caused and fixed for real this time rather than worked around again: `layout.tsx`'s SW registration script now only calls `.register()` when `process.env.NODE_ENV === "production"`, and proactively unregisters any SW found in every other environment, so local dev can no longer inherit this failure mode at all (new AGENTS.md Rule 66). Documented the visual rework as Rule 65. `tsc`/lint/30 tests all clean, live-verified in-browser after a full clean restart with 0 service-worker registrations confirmed, Fraunces font-family confirmed resolving (not silently falling back per the Rule 49 failure class).)
> Last audited: 2026-08-13 (Session 79 - fixed Mermaid diagram rendering layout, sizing, and blank space issues. Enhanced the `tightenViewBox` function in `src/components/Mermaid.tsx` to correctly target the root diagram container, avoiding helper `<g>` elements inside `<defs>` (which have tiny or 0 dimensions and caused incorrect viewBox cropping, resulting in blank/distorted flowcharts). Added a client-side `useEffect` that performs layout-dependent `viewBox` adjustment directly in the DOM using `requestAnimationFrame` on both inline and fullscreen containers, ensuring accurate `getBBox()` calculations once elements are mounted and visible. Conducted a full project audit for layout and SVG rendering issues, identifying a dormant text-overflow hazard in the DiagramBlock component where standard SVG text elements do not wrap. Upgraded DiagramBlock's funnel, bars, timeline, cycle, and flow diagrams to use SVG foreignObject containers with HTML/CSS text wrapping, ensuring full responsiveness and no label clipping when used in future lessons. Verified all tests, lint, and build pass clean.)
> Last audited: 2026-08-13 (Session 78 - two user-driven changes, no lesson content touched. (1) Investigated a user report that a Mermaid diagram on the live site rendered too small to read; a fresh browser tab against the actual production deployment rendered it correctly (SVG filled 93.5% of its container, same as local dev), and the tab that showed the bug had a live `ma-v2` service-worker cache registered — confirmed root cause is the same stale-bundle mechanism AGENTS.md Rule 50 already documents for post-deploy dev sessions, now confirmed to also affect any long-lived production tab across a deploy, not just local dev. Made one small defensive hardening anyway while investigating: `Mermaid.tsx`'s SVG sizing used `max-w-full` (caps size, never scales up), so a diagram whose Mermaid-computed natural size is smaller than its container would never stretch to fill it; changed to `w-full` (both the inline view and the fullscreen overlay) so diagrams always fill their container regardless of natural size. Not the actual cause of the reported bug, kept as a genuine improvement. (2) Built a dedicated project detail page per explicit user request ("each Project should open in a new page similar to lessons, both opening in new tab & separate page, irrespective if opened from lessons or project page"). New statically-generated route `src/app/projects/[category]/[slug]/page.tsx` (all 98 projects, verified via `npm run build`) plus `src/lib/projects/lookup.ts` (server-side project-by-id lookup, mirrors the old `ProjectDrawer.tsx`'s dynamic-import + export-name-fallback logic). `ProjectCard.tsx` gained a `variant` prop: `"preview"` (used in `ProjectList` on the lesson page) now links out via `target="_blank"` instead of expanding inline; `"full"` (used only on the new page) always renders the body. The `/projects` hub's "View details" button now links to the same dedicated page instead of opening a slide-over drawer; `ProjectDrawer.tsx` deleted as dead code. Then, per a follow-up request with explicit scope confirmed via clarifying questions (listing/hub pages + in-lesson links, NOT primary nav, NOT category-only links), extended `target="_blank" rel="noopener noreferrer"` to every lesson-destination link found across a full-repo grep: `/learn` index, `/learn/[category]`, homepage (hero CTA, featured lessons, recent lessons, bottom CTA), search results, cheat-sheets, interview-prep, bookmarks, recently-viewed, track lesson lists, and both projects-page lesson backlinks (17 links total, verified none of the 5 category-only links or the primary nav's own "Start Learning" link were touched). `LessonResourcesClient.tsx`'s external resource links (the actual "Free Resources" section under a lesson, `ResourceList.tsx` itself is a suppressed no-op per an earlier session) were already `target="_blank"`, confirmed not changed. Verified: `tsc --noEmit` clean, `npm test` 30/30 clean, `npm run build` clean (all 98 project detail pages + all existing routes generated), live-checked in-browser (project links open in a new tab from both the lesson page and the hub, dedicated page renders full teardown/answer-key content, category and nav links unaffected).)
> Last audited: 2026-08-12 (Session 77 - executed PROJECTS_PLAN.md Stage 8.3b's first half: AI Search Optimization track, all 14 lessons (12 `seo` + 2 `ai-marketing`), fully done — content-quality rewrite + 2 projects + 2 `InAction` scenarios each, per the owner's expanded 8.3b scope. Branch `stage-8-3b-ai-search-optimization`. Execution: 7 parallel `general-purpose` agents (Agent tool, no Workflow opt-in this session), 2 lessons/agent per PROJECTS_AUTHORING_GUIDE.md's Rule-56 batch size, each agent doing the rewrite first then authoring projects/scenarios against the final headings in one pass (8.3b's stated hazard order). New `src/lib/projects/ai-marketing.ts` category file created (first projects in that category). All 14 lessons refreshed with 2025/2026-dated research (word counts up 200-500 words each, genuine added depth: comparison tables, corrected stats, new worked sections, not padding), `relatedConcepts` added to the 4 lessons missing it, every rewrite re-checked against its existing 4-question quiz with zero conflicts found. One quiz/lesson **pre-existing** mismatch flagged, not fixed: `seo/zero-click-search`'s quiz option says "~75-80% of searches" while the lesson has always said ~68%, needs a `quizzes.ts` edit outside this session's scope. One factual bug in existing content found and fixed: `entity-seo.mdx`'s HubSpot case study cited the wrong Wikidata Q-number (Q5928322, an unrelated Iranian heritage site) for HubSpot, corrected to the verified real one (Q5926631). Merge: `scripts/merge-projects-batch.mjs` run once for `seo` (6 scratch files -> 15->27 lesson keys) and once for the new `ai-marketing.ts` (1 file -> 2 keys); caught and fixed one new merge-script edge case, one agent wrote its first lesson key unquoted (`aeo: [` instead of `"aeo": [`, both valid JS but the script's regex-based key-counter only recognizes quoted keys), which silently under-counted the merge log by 1 without breaking the actual file — see AGENTS.md Rule 58. `scripts/audit-projects.mjs` clean on both categories, `tsc`/`lint`/`npm test` (30/30) all clean, full `npm run build` clean (958 static pages), `/projects` hub index regenerated (60->88 project cards across 45 lessons), live-checked `seo/aeo` and `/projects` in-browser (InAction blocks, "2 projects · ~75 min total" summary, Related Concepts cards, no console errors). Scheduled a one-time check-in task (`projects-stage-8-3b-checkin`, fires 2026-08-13 03:47 IST) to continue Stage 8.3b's second half (On-Page SEO Mastery's 5 remaining lessons + rewrite pass on its other 8) if this session isn't still active. Pushed to `stage-8-3b-ai-search-optimization` on request, in batches as authored, not one giant merge at the end. **Then completed Stage 8.3b's second half in the same session**: On-Page SEO Mastery, all remaining work — 5 lessons got new projects (`search-intent`, `image-seo-visual-search`, `video-seo` in `seo.ts`; `headlines` in `copywriting.ts`; `blog-seo-content` in `content.ts`, the first entries in that file's `blog-seo-content` key), plus a rewrite-and-InAction-only pass (no new projects, since they already had them from the Session 73 pilot) on `on-page-seo` and `keyword-research`. 5 parallel agents, 7 lessons total. Found and fixed a SECOND real instance of the Rule 58 unquoted-key bug, pre-existing this time, not introduced this session: `copywriting.ts`'s `headlines` key had been written unquoted (`headlines: [`) in an earlier session, invisible to `get-track-batch-info.mjs`'s regex-based "already has projects" check, so it was wrongly listed as one of the "5 remaining" lessons needing new projects. An authoring agent caught the conflict itself (found the existing 2 projects by reading the file directly rather than trusting the script) and flagged it instead of silently duplicating. Fixed by quoting the key and manually appending the 2 new projects (mini/teardown, core/build) into the existing array rather than running the merge script, giving `headlines` 4 total projects across 4 distinct archetypes (head-to-head, build-the-asset, teardown, reverse-engineer) — verified by the archetype-reuse test in `npm test`. Repo-wide grep for any other unquoted top-level keys came back clean. `image-seo-visual-search` (the flagged thinnest lesson at 1,092 words) grew to 1,861 words with 2 new structural sections (format-comparison table, worked example). All rewrites re-checked against existing quizzes; one closed an actual content gap (`keyword-research`'s zero-click quiz question now has supporting lesson content, previously tested a fact the lesson never taught). `scripts/audit-projects.mjs` clean across all 3 touched categories, `tsc`/`lint`/`npm test` (30/30)/`npm run build` all clean, `/projects` index regenerated (88->98 cards across 50 lessons, the true prior count was 46 not 45, the hidden unquoted `headlines` key was invisible to the same regex used for that stat), live-checked `copywriting/headlines` in-browser ("4 PROJECTS" badge confirmed). Both halves of Stage 8.3b are now done: 27/27 lessons across AI Search Optimization + On-Page SEO Mastery.)
> Last audited: 2026-08-12 (Session 76 - PROJECTS_PLAN.md Stage 8.3 (Phase 2 projects) + 8.4 (concept scenarios) for the Technical SEO Mastery track, 12 of its 13 lessons (`mobile-first-indexing` already done in Session 74). Scope decided interactively with the owner: all `seo`-category lessons across the 4 SEO-named tracks total 30 remaining, checkpointed down to Technical SEO Mastery's 12 first; the other 3 tracks' 18 lessons and the 11 cross-category lessons inside those same tracks are explicitly deferred (PROJECTS_PLAN.md 8.3a). 8.4 required building the concept-scenario feature from scratch, zero code existed for it before this session (confirmed by grep): new `src/components/InAction.tsx` global MDX component, registered in root `mdx-components.tsx`. Deliberately skipped the plan's preferred build-time rehype-injection design (10.5) in favor of its own explicitly-sanctioned direct-MDX-embed fallback, since `AGENTS.md` Rule 10 already documents this repo's `@next/mdx`/Turbopack setup breaking on non-string-tuple plugin forms and a novel rehype transform was judged too much build-break risk for one session (AGENTS.md Rule 54). Execution: 3 parallel `general-purpose` subagents (Agent tool, not Workflow, no multi-agent-orchestration opt-in), 4 lessons each, each researching real citations via WebSearch/WebFetch and authoring both 2 projects and 2 `InAction` scenarios per lesson in one pass. 24 new projects merged into `src/lib/projects/seo.ts` (now 15 lessons / 30 projects in that file, 60 across the whole `/projects` hub), 24 scenarios embedded directly into the 12 lessons' MDX. Review gate: `tsc`/`lint`/`npm test` (18/18) all clean; a structural audit script found 4 projects using invented placeholder tool names instead of a real `tools-directory.ts` entry, fixed by adding 2 real, previously-uncataloged tools (`Google Sheets`, informally used 20+ times across `email.ts`/`fundamentals.ts`/the original pilot's own `keyword-research` project without ever being added to the directory it's a foreign key into, and `Cloudflare`) rather than inventing more placeholders (AGENTS.md Rule 55); company-variety and archetype-variety checks both clean (no company over 2×, no lesson repeats an archetype in its own pair); 6 of the 24 citations independently spot-checked via WebFetch/WebSearch against their claimed sources, all 6 confirmed exact. Two mechanical mistakes caught before landing: a hand-written merge script's regex silently mishandled 2 of 3 batches' lesson keys on the first attempt (caught by grepping the merged file's key list, not by `tsc`, since a `Record` with misplaced keys can be syntactically valid with the wrong runtime shape), and a leftover `.bak` file inside `src/lib/projects/` broke the Turbopack build entirely until deleted (that directory is dynamically globbed). `npm run build` (958+ static pages) and a live browser check (schema-structured-data's `InAction` card + "2 projects · ~65 min total" summary) both confirmed clean. Also had to `npm install` mid-session: the session's earlier `git pull` had updated `package-lock.json` (a large Stage 8 projects-pilot merge from a separate contributor branch) without `node_modules` ever being synced to it, so `tsx` was missing and `npm test` couldn't run at all until fixed; unrelated to this session's own content changes. At the owner's request, analyzed the 3 agents' actual token cost (267k/280k/282k tokens for 4 lessons each) and identified 2 concrete, non-quality-degrading causes: duplicated reference-file reads (each agent independently re-reading the same ~4,300 lines of `types.ts`/`case-companies.ts`/`tools-directory.ts`/examples) and context compounding within a single multi-lesson agent session (every tool call re-sends the whole conversation so far). Recorded as AGENTS.md Rule 56 and PROJECTS_PLAN.md's Stage 8.3a note. Then, per an explicit follow-up request to make the improved process reusable rather than just documented, built it for real: `PROJECTS_AUTHORING_GUIDE.md` (the operational playbook, fill-in agent-prompt template with the condensed reference pack inlined) plus 3 tested scripts, `scripts/get-track-batch-info.mjs` (replaces manually grepping `tracks.ts`/`projects-assignment.ts`, verified against all 3 remaining SEO tracks, output matched the manually-derived 18-lesson count exactly), `scripts/merge-projects-batch.mjs` (fixes the exact regex bug hit above at the root, plus new safety: refuses duplicate lesson keys, verifies final key count, backs up outside `src/lib/projects/` so the `.bak`-file build-break above can't recur; tested against a synthetic fixture, clean merge and duplicate-rejection both verified), `scripts/audit-projects.mjs` (generalizes the one-off `seo.ts`-only audit into any category, scoped or whole-file). Running the whole-file audit surfaced a real pre-existing finding outside this session's own new content: the original Session 73 pilot's `keyword-research`/`on-page-seo` projects have the identical Rule 55 placeholder-toolName issue, never caught before this script existed. Flagged in `PROJECTS_PLAN.md` 8.3a, not fixed (out of scope this session). Final pass: cross-linked every new/touched file so the docs form a closed loop, `PROJECTS_PLAN.md` 8.3a and `AGENTS.md` Rule 56 both point at the guide and scripts by name, the guide points back at the spec/rules, all 3 scripts reference each other's place in the sequence plus the guide, `README.md`'s Key Files table lists all 4 new files (also fixed a wrong rule-number citation, "Rule 49", introduced in the same session's own earlier README edit — should have been Rule 54). Two commits pushed to `feat/stage8-technical-seo-track`: content (21 files) then tooling (4 files) then this cross-link pass. Not yet run against real content — the guide's own section 4 has a placeholder for the first real measured token/lesson number once Stage 8.3a's lessons actually get authored. **Post-push correction**: re-ran the remaining-lessons check against every track referencing a `seo`-category lesson, not just the 4 explicitly SEO-named ones, and found the "18 remaining" figure undercounted by 1 — `content-decay-refresh` is only reachable via the non-SEO-named Content Strategy Mastery track. True total is 19, corrected in `PROJECTS_PLAN.md` 8.3a, this file's resume line, and `README.md`. **Then, at the owner's request, listed all 24 tracks with real per-track completion counts (queried live from src/lib/projects/*.ts + tracks.ts, not recalled from memory), and the owner set an explicit priority order covering all categories, not just SEO: AI Search Optimization, AI-First Marketer, Off-Page SEO Mastery, On-Page SEO Mastery, B2B Marketing, Paid Ads Mastery, Data-Driven Marketer, CRO & Conversion Mastery, Analytics & Measurement Mastery, Email & Lifecycle Mastery (owner's explicit 1-10), then "rest based on importance" for the remaining 13 — proposed an order weighted toward finishing near-complete tracks first (Solo Founder needs exactly 1 lesson) then cross-cutting skills before specialized ones, flagged as assistant-proposed and needing confirmation. Recorded as PROJECTS_PLAN.md 8.3a's new priority table, which now supersedes the SEO-only framing (kept below it as still-valid detail for whichever SEO tracks come up in the queue).** **Finally, a review pass over PROJECTS_AUTHORING_GUIDE.md + PROJECTS_PLAN.md + their related files, checked against actual source rather than the docs' own claims, which found and fixed 7 real defects: (1) `get-track-batch-info.mjs` built batches ignoring category, so on the very next track in the queue (`ai-search-optimization`, which spans `seo`+`ai-marketing`) batches F and G each mixed two categories while `merge-projects-batch.mjs` accepts exactly one, making those scratch files unmergeable, fixed to group per category and print the per-category merge commands; (2) the guide's condensed reference pack omitted `SimulationStage`/`LiveTrack`/`Verdict` and `Project.stages`/`liveTrack` entirely, so 4 of the 7 modes were invisible to authoring agents and priority-#6 Paid Ads Mastery (for which simulation is structurally THE mode, per the plan's own section 2.3b) would have been authored wrong, fixed by adding the types, a mode->array table, an explicit `no-project` note, and a trimmed real simulation example as section 3.2b; (3) the guide's worked example had meta-commentary sitting inside the `sampleOutput` data field, teaching agents to write instructions into data, replaced with a real rendered sample; (4) PROJECTS_PLAN.md's header still read "Status: PROPOSED, no code written yet... ~120 findings, none implemented" and named `/api/groq` as the single most urgent live issue when that route was deleted back in Session 71, rewritten to state real current status with a historical-note banner; (5) `scripts/audit-projects.mjs` had a false-positive bug of its own, it isolated a lesson's body by scanning for a `\n  ],\n` terminator, which an inline empty `no-project` entry (`"what-is-marketing": [],`) has none of, so it silently swallowed the NEXT lesson's projects and validated them against the wrong lesson's MDX headings, producing confident wrong "lessonAnchor is not a real heading" failures, fixed to slice key-to-next-key and to count `no-project` verdicts explicitly; (6) with that fixed, the sweep found the remaining real Rule 55 violations across ALL categories, not just seo, 9 placeholder `toolName` strings in Session 73 pilot content, each replaced with the real tool already named in its own `where` field (`Manual page crawl`->`Screaming Frog SEO Spider`, `Manual calculation`->`Google Sheets`, `Written justification`->`Notion`, `Manual rewrite`/`Manual edit`->`WordPress`) plus 3 genuinely-uncataloged real tools added to `tools-directory.ts` after verifying each is live and free via WebFetch (`Google Docs`, `CoSchedule Headline Analyzer`, `Advanced Marketing Institute Headline Analyzer`, 113->116 tools); (7) new `tests/projects-data.test.ts` (12 assertions, 18->30 tests) now enforces all of it in `npm test`/CI on the real imported objects rather than a text scan, since every one of these invariants is a valid string to `tsc` and none of them fail a type check, which is exactly how the pilot's placeholders survived three sessions. Two candidate assertions were written and deliberately REMOVED because they failed on legitimate content (a "no trailing period" style heuristic on `conceptsCovered`, and an exact `conceptsCovered`-must-match-`step.concept` rule that breaks on the 6 of 33 projects that reasonably shorten a long step concept into a readable card label), recorded in AGENTS.md Rule 57 along with the empty-array parsing trap. Test verified to actually catch regressions by injecting a bad companyId/toolName/lessonAnchor and confirming all three assertions fire with the offending project id. Also corrected stale counts found in passing: README's features list said 216 glossary terms (real: 158) and 108 tools (real: 116), and both README and the resume line said 18 tests.** Merged as PR #12 (squash) after CI build passed. **Then scoped, but did NOT execute, the next two tracks at the owner's direction (PROJECTS_PLAN.md 8.3b): AI Search Optimization (14 lessons, 0 with projects, 7 per-category batches with tier pairs listed) and On-Page SEO Mastery (13 lessons, 8 already done, 5 remaining of which 2 are cross-category into copywriting.ts/content.ts). Owner expanded scope beyond projects+scenarios to a full content-quality rewrite of all 27 lessons, and chose AI-Search-first sequencing. Measured rather than assumed: no lesson is a stub (1,092-2,183 words), `keyword-research` is the one stale-year lesson, `image-seo-visual-search` is the thinnest at 1,092 words / 5 h2 headings, 9 lessons lack `relatedConcepts`, only 5 of 27 already carry `InAction` blocks. Ran the Rule 34 audit across all 74 seo+ai-marketing lessons: no new lesson is justified, both tracks are saturated, but `seo/agentic-commerce-seo` exists and is missing from the AI Search track and should simply be added to `tracks.ts`. Recorded 4 hazards this batch carries that the Technical SEO one did not: rewriting already-good lessons can degrade them; all 14 AI Search lessons have 4-question quizzes a rewrite can silently invalidate and nothing tests that; changing a heading breaks `lessonAnchor`s (harmless for AI Search's 0 existing projects, a live hazard for On-Page's 8 done lessons, now caught by `npm test`); and the roster is over-concentrated with `freshworks` at 9 uses and `chewy` at 5 against a ~4 cap while 35 of 77 companies sit completely unused.**)
> Last audited: 2026-08-12 (Session 75 - lesson reader page redesign, explicit user request via a multi-round design review: built 4 mockup layout variations as a single self-contained HTML Artifact (rail+tabs, guided-journey stepper, dashboard-sidebar, and a combined rail+dashboard) using seo/mobile-first-indexing as the live test content, then 3 more grounded explicitly in named reference products (Stripe Docs/MDN/GitBook-style, Duolingo/Coursera-style course player, Medium/Substack-style editorial) after the user rejected the first 4. User approved the docs-style variation with 2 amendments (ToC moved to the left instead of right margin, "Related Concepts" pulled out of the accordion group into its own card grid) plus removing the page's "Back to {category}" link entirely, then had it built for real. Implementation: `TableOfContents.tsx`'s desktop ToC restyled from a bordered 240px rail to a thin 176px scroll-spy dot list; the header's scattered LevelBadge pill + Clock icon + separate spans collapsed into one font-data meta line; Mark Complete/Bookmark/Share moved out of the header into one action bar at the very bottom of the page; Quiz wrapped in a native `<details id="quiz-accordion" open>` (MarkComplete's locked-click handler now force-opens it before `scrollIntoView`, since a closed `<details>` has no box to scroll to); ProjectList and LessonNotes, which already self-collapse, restyled to match the same card language rather than double-wrapped. New `src/components/RelatedConcepts.tsx` finally reads `lessonMeta.relatedConcepts`, a field written into ~65 lessons' MDX since early sessions but never consumed by any component until now, resolving each slug against real `curriculum.ts` data and rendering cards above the existing algorithmic "You Might Also Like" (`RelatedLessons.tsx`, left unchanged, the two are additive not redundant). Verified: 18/18 tests, tsc/lint clean, full build clean (958 static pages incl. all 655 lesson pages), live-checked in-browser on seo/mobile-first-indexing after a full dev-server restart (hit the known Turbopack/Windows staleness issue from Rule 50 mid-verification, a `ReferenceError: Clock is not defined` from a pre-restart stale chunk that cleared on `rm -rf .next` + fresh tab, not a real bug — the import had already been correctly removed). A pre-existing dev-mode-only hydration/script-tag console warning was confirmed to reproduce identically on an untouched lesson page, so it predates and is unrelated to this change.)
> Last audited: 2026-08-12 (Session 74 - continued from Session 73's pilot. Nav/footer decluttering: Nav consolidated 5 top-level dropdowns to 4 (Topics/Learn/Resources/About), folded the thin "Progress" dropdown into "Learn" as a labeled section, dropped Bookmarks/Search (already have dedicated icon buttons), split Resources into Reference/Tools; Footer trimmed the all-21-category two-column listing down to a single 6-category column + "browse all" (full browsing already lives in the Nav mega menu), Learn 9->6 links, Resources 8->6. Stage 8.2b: computed PROJECTS_PLAN.md section 17's centrality band + tier-pair matrix for all 642 lessons via scripts/compute-project-assignment.mjs (src/lib/projects-assignment.ts, generated), verified exactly against the plan's own cited top-12 hub list (all 12 track-reference counts matched, e.g. analytics-101=6, utm-tagging=5). Attempted Stage 8.3 (Phase 2, 226 remaining lessons) as a large multi-agent Workflow; the first launch failed instantly on a workflow-VM boundary limit (a 25KB args payload exceeded a 4096-unit cap on large strings crossing into the sandbox), fixed by having agents self-discover their lesson slice from projects-assignment.ts instead of receiving pre-computed data; the user then asked about cost-effectiveness before letting it run (batch size 3 lessons/agent -> 82 agents was flagged as needlessly redundant "read background files" overhead vs. a coarser batch), and the run was stopped pre-execution (zero files written, no loss) in favor of validating quality on Google's Antigravity (Gemini) as a cross-tool test first: one lesson (seo/mobile-first-indexing, 2 projects) authored there against the same hard-rules prompt, independently verified (tsc/lint clean, dataset numbers traced exactly to the real CSV, concepts traced to real MDX headings), 2 minor convention gaps fixed (free-tool required:true->false, missing paidUpgradeNote) before merging into seo.ts (34->36 project cards). Stage 8.3's remaining 225 lessons are NOT done. Visual redesign (explicit user request, colors deliberately UNCHANGED from the original white/near-black/indigo-#6366f1 palette, structure+typography only): a "field manual" identity for Home, /learn, /tracks, /projects, /tools, /about, grounded in the site's own stated differentiator (cited research, ordered curriculum) rather than a generic SaaS template or one of the three common AI-design defaults. New type system (Fraunces display / Public Sans UI / IBM Plex Mono data, registered as Tailwind v4 @theme inline tokens in layout.tsx+globals.css; lesson-reading prose intentionally stays on Geist Sans). New shared components: PageMasthead.tsx (journal-masthead info strip on all 6 pages), classificationCodes.ts (2-4 letter discipline codes used as specimen-card eyebrows). Signature element: "field notes" marginalia on Home's "Not like the others" section, citation marks + green verification annotations tied to real evidence. Hit and fixed a Tailwind v4 @theme-inline bug during build: a theme key aliasing to a DIFFERENTLY-named CSS variable (`--font-data: var(--font-data-mono)`) silently failed to resolve at runtime while a key aliasing to its OWN name (`--font-display: var(--font-display)`) worked; fixed by renaming the underlying next/font variable to match directly, eliminating the cross-reference. Also hit repeated dev-server staleness after edits (Turbopack's Windows file watcher not triggering recompiles, and the site's own service worker aggressively caching old JS bundles); reliable fix is a full server restart (stop, rm -rf .next, restart) plus a genuinely fresh browser tab, not just a page reload. Lesson-page reorganization (explicit user request, "too many elements"): removed a redundant "Up Next" card that duplicated both MarkComplete's own post-completion "Continue" CTA and the Prev/Next nav at the page bottom (3 copies of the same information down to 1); made the whole Practice Projects section collapse behind a one-line summary button by default ("2 projects · ~70 min total · reinforces X"), not just each individual ProjectCard's own inner toggle; grouped Notes/tool-comparison-callout/Related-Lessons/Prev-Next-nav/back-link under one lighter-weight wrap-up cluster instead of each getting its own full `mt-12 pt-8 border-t` section treatment. Verified: 18/18 tests, tsc/lint clean (lint went from 1 pre-existing warning to 0, incidentally fixed while rewriting TracksPageClient.tsx), full build clean (958 static pages), all pages checked live in-browser including the Projects-collapsed-by-default behavior and its expand toggle.)
> Last audited: 2026-08-12 (Session 73 - executed PROJECTS_PLAN.md Stage 8 Phase 0 + Phase 1 pilot via a 3-stage multi-agent Workflow pipeline (roster/types/datasets -> component layer+hub -> content authoring+review-gate), on branch `feat/stage-8-projects-pilot`. Phase 0: `src/lib/projects/types.ts` (full type system, 6 modes + no-project verdict, 9 archetypes), `src/lib/case-companies.ts` (77 verified companies with cited exits, 66/34 global/india, 38 mega/27 large/5 mid/7 small scale, after a gap-fill pass corrected an initial 74/26 mega-only skew), 8 starter datasets in public/project-data/. Phase 1: full component layer (ProjectList/ProjectCard/ProjectStep/SimulationRunner/SimulationDebrief/LiveTrackPanel/TeardownItemCard/ToolStack/OutputSample), src/lib/projects-progress.ts (localStorage+XP, project=40/bigProject=100 added to engagement.ts XPAction), TableOfContents.tsx extraSections prop (also fixed the pre-existing Quiz-missing-from-ToC gap), /projects hub mirroring ToolsClient.tsx, scripts/build-projects-index.mjs generator. Content: 34 projects across 9 category modules covering all 19 pilot lessons (18 solo-founder track lessons + paid-ads-101) plus 4 track-level big projects, authored on Sonnet 5 at high effort with an Opus 5 medium-effort review-gate agent per the plan's own explicit acceptance criteria (the diagnostic 8-question test and the simulation branching-consequence test from PROJECTS_PLAN.md section 2.3a/2.3b). Review-gate verdict: PASS-WITH-CONCERNS, found the paid-ads-101 simulation's costly branch spent less and cost less per conversion than the optimal branch (backwards), 5 dead lessonAnchor values pointing at prose not headings, 19 conceptsCovered entries written as sentences instead of concept names, one tool-name typo breaking a lookup, and TeardownItem specimen/answerKey/distractors data that was authored but never rendered (ProjectCard only showed item.prompt). All defects fixed directly: rewrote the simulation's two costly terminals with genuinely worse, internally-consistent economics (verified live in browser: 2 conversions/£285.70-per-conversion/CTR-below-industry-average vs the optimal path's 4/£149.81/above-average), fixed all anchors, normalized conceptsCovered to derive from step.concept per a new AGENTS.md Rule 46, fixed the typo, built TeardownItemCard.tsx with a reveal-gated answer key (new Rule 48). A regression-test false positive also surfaced and was fixed (Rule 47, the Rule 36 gate test does literal text matching and flagged a comment mentioning progress.ts's markComplete() by name inside projects-progress.ts, an unrelated function). Final state: 18/18 tests pass, tsc/build/lint all clean, full pipeline verified live in the browser (paid-ads-101 lesson page renders 2 projects with a working simulation and corrected branching, /projects hub renders all 34 cards with working filters and a lazy-loaded drawer). AGENTS.md Rules 45-48 added. Stage 8.2b (centrality-band computation across all 642 lessons) and 8.3/8.4 (Phase 2 scale, concept scenarios) remain, explicitly gated behind a human walkthrough of this pilot per the plan's own stated review-gate.)
> Last audited: 2026-08-12 (Session 72 - continued from Session 71. Completed remaining PROJECTS_PLAN.md stages: 3.5 IntersectionObserver gating for Mermaid (-135KB gzip deferred), 3.6 @next/bundle-analyzer integration, 3b.5 GitHub Actions CI (lint+test+build), 4.1 heading hierarchy fix (80 headings across 10 MDX files), 6.2 ARIA combobox for CommandPalette, 6.4 Mermaid text alternatives, 7.7 homepage entry-point reduction (9->7), 7.8 mobile ToC max-height + quiz pill overflow, 9.4 UTF-8 BOM cleanup (19 files). Fixed two dev-mode console errors: (1) PostHog Turbopack crash - removed all posthog-js npm imports, PostHogProvider is now a no-op wrapper, PostHog loads via CDN snippet only when NEXT_PUBLIC_POSTHOG_KEY is set; MarkComplete and LessonViewTracker use window.posthog?.capture() instead; (2) Script tag React warning documented as unavoidable for inline scripts, added suppressHydrationWarning on html element. Deleted PostHogInit.tsx. Confirmed Stages 2.2-2.8 already implemented in prior session. All stages 0-7 and 9.4 now complete. Stages 8-10 remain as future content/feature work.)
> Last audited: 2026-08-12 (Session 71 - executed PROJECTS_PLAN.md Stages 0-7 across 9 commits on branch `fix/stage-0-p0-safety-fixes`, merged to main via PR #11. Key deliverables: deleted unauthenticated /api/groq LLM proxy; disabled cloud sync UI until configured; gated certificates on 100% + track quiz pass; try/catch on all layout-level localStorage calls; full quiz integrity family (checkboxes gated on quiz pass, answers hidden until submit, Fisher-Yates shuffle with correct-index recomputation, per-lesson ≥50% minimum on track quiz, markIncomplete reverses XP); sourceCategory ID resolution (fundamentals uncapped, achievements reachable); localStorage corruption recovery with StorageWarning UI; CATEGORY_INDEX slim projection for Nav (-48KB gzip); lazy DOMPurify + PostHog; /learn collapsed to 5/category; test infrastructure (18 tests, 6 suites, Node built-in runner + tsx); stale-count fixes (393→642, 15→21, 22→24); 306 copy artifacts cleaned across 53 MDX files; focus-visible + reduced-motion + aria-live + aria-pressed; 13 nested main landmarks fixed; track quiz pagination (10/page + sessionStorage); empty-state nudges; mobile StreakBadge; certificate print CSS; onboarding "totally new" option + lesson-page suppression. Stage 4 (content: headings/slugs/stubs) deferred as research-required. Stages 8-10 (projects layer, long tail, quiz expansion) are future content work.)
> Last audited: 2026-08-11 (Session 70 - PLANNING ONLY, no feature code written. Produced PROJECTS_PLAN.md (~1,600 lines), the high-priority roadmap for a hands-on projects layer across all 642 lessons, a /projects hub, and per-concept scenarios in lessons. Key research: a four-agent survey of 454 lessons across all 21 categories found the initial three-mode project model (diagnostic/simulation/build) covers only ~73% of the library; all four agents independently converged on the same missing 4th mode (teardown, supplied specimen + fixed expert answer key), and mental-models needs a 6th (calibration, deferred verification) because simulation's instant feedback is structurally opposed to lessons about delayed consequences. Also ran a full mechanical audit of all 642 lesson files, verified findings recorded in PROJECTS_PLAN.md section 12: 53 files carry a visible " ,  " em-dash-replacement artifact, 10 lessons have zero ## headings so no TOC renders at all, 'Related Concepts' exists in only 63/642 (10%), 45 lessons cite 2024 and never 2025/2026, 19 files carry a UTF-8 BOM, 25 use single-quoted lessonMeta, 4 lessons are under 700 words. Two false positives recorded so they are not 'fixed' by mistake, notably the 165 files writing Mermaid labels as A[Line\nLine] which is the CORRECT pattern per Rule 30. CRITICAL FINDING: confirmed a live P0 integrity bug, TrackLessonList.tsx per-lesson checkboxes call markComplete() + addXP('complete') with no getQuizPassed() check, so a learner can mark a whole 13-lesson track complete and collect 390 XP plus a certificate in ~5 seconds without opening a single lesson. AGENTS.md Rule 24 documented the ungated behaviour as intentional and now contradicts Rule 25; Rule 24 has been corrected in place and new Rules 36-39 added. Rechecked the whole integrity surface afterwards and found FOUR MORE related defects, all confirmed in code and documented in PROJECTS_PLAN.md 0.1b-0.1f: (2) certificates/[slug]/page.tsx computes completion pct and never gates on it, so anyone can print a Certificate of Completion at 0% just by visiting the URL, no clicks needed, (3) Quiz.tsx reveals the correct answer plus explanation per question then offers unlimited retry, so 100% is reachable in ~30 seconds without knowledge, which matters because fixing bug 1 makes the quiz the only gate, (4) TrackQuizPageClient passes at 80% of a POOLED question set so a learner can score 0% on two lessons and still have them marked complete, (5) markIncomplete() does not reverse XP while addXP dedupes on a rolling 24h window, making completions farmable. Root cause is shared: completion state is written from three places and only one checks anything. All five must be fixed as one batch, fixing one alone moves the hole rather than closing it. Execution order is in PROJECTS_PLAN.md section 0. THEN ran a five-lens adversarial CODE audit (security/abuse, state integrity, accessibility, performance, new-learner UX) via 5 parallel agents, each required to cite file:line and separate confirmed from unconfirmed, recorded in PROJECTS_PLAN.md section 14: ~94 further findings, 13 of them P0. Worst three: /api/groq is an unauthenticated LLM proxy on GROQ_API_KEY with ZERO callers anywhere in the app (dead code, live, uncapped billing exposure); /api/sync-proxy writes every user to one hardcoded global KV key "progress" with no user identity, so any visitor can read another user private ma_note_* content or wipe all synced progress, and its only gate is NEXT_PUBLIC_SYNC_SECRET which Next inlines into a public JS chunk; and ThemeToggle/OnboardingModal/LessonNotes call raw localStorage with no try/catch while rendering inside layout.tsx, so blocked site data shows Next raw crash screen on EVERY page (no global-error.tsx exists). Also confirmed: fundamentals is permanently capped at 27/40 because 13 cross-listed lessons are written under sourceCategory but read under fundamentals, and the all-lessons achievement needs flatLessons().length=655 while max writable is 642, making Marketing Polymath unreachable by exactly 13 for everyone. Verified those counts directly (655 entries / 13 cross-listed / 642 unique = the 642 mdx files on disk) after the state agent misreported them as 606/593. Performance audit DISPROVED this plan own section 1.3: quizzes.ts (1.91MB) and lesson-resources.ts (1.72MB) do NOT reach the client (grepped all 105 chunks, 0 matches) because Quiz.tsx takes questions as a prop and the lesson page is a server component; the real bundle bug is Nav.tsx being "use client" and importing CATEGORIES, costing 48KB gzip on every route. Section 0 re-prioritised into 10 stages ordered by live harm. AGENTS.md Rules 41-44 added. Then rechecked the plan itself and found 3 defects, 2 self-inflicted: section 1.3 still asserted the "3-4MB to the client" claim the perf audit had disproven, with no pointer to the correction 2,000 lines later (fixed in place with a banner, section kept because the error is instructive); the plan proposed rule numbers 20-28 and 41-48 that collide with AGENTS.md real rules 1-44 (noted as draft labels, renumber from 45 at merge); and there is NO test infrastructure at all (no test script, no test files), which makes plan item 1.8 unimplementable as written. Added section 15 (testing and data validation, reframed around data assertions over static files rather than component tests, ~80% of value from node --test with zero new deps, includes a property test that shuffling preserves Quiz.correct across all 2,252 questions) and section 16 (11 open decisions blocking implementation, each with options and a recommendation, covering whether to delete /api/groq, whether to disable cloud sync, migration for bypass-earned completions, quiz thresholds, certificate bar, and answer-reveal timing). Section 0 gained Stage 3b for the test work. Finally added section 17 (how projects get assigned to lessons) after measuring that lesson difficulty INVERSELY correlates with lesson importance: Beginner lessons average 0.98 track references vs 0.46-0.47 for Intermediate/Advanced, and ALL TWELVE of the most-referenced lessons in the curriculum are Beginner (analytics-101 in 6 tracks, email-marketing-101 and utm-tagging in 5, keyword-research/on-page-seo/content-strategy/copywriting-101/headlines/meta-ads in 4). So the intuitive rule easy-lesson-gets-easy-project would give the most trivial treatment to the twelve lessons everything else depends on. Replaced it with a two-axis model: cognitive demand from level (what the project asks) x investment weight from centrality (how much it asks), with a 3x3 assignment matrix whose load-bearing cell is Beginner x Hub = two core projects. Also derived archetype selection from topic shape rather than rotation, and found a budget problem: applying the matrix naively to an average track (15.1 lessons) yields ~88 hours of project work against an advertised duration averaging 15.4 hours, a 5.7x overrun, so track durations must split reading vs practice time and cap core projects per track. THEN audited the plan against itself with 2 more agents (contradictions/unverified-claims, and missing-specifications), recorded as section 18: 62 findings, 8 P0. Six factual errors fixed inline, worst of which: the India feasibility table conflated Swiggy IPO raise with valuation, printing "~$11.3B" where ₹11,327 Cr is ~$1.36B, an 8.3x error INSIDE the section written to enforce currency discipline; the "no company in more than 4 projects" quota is arithmetically impossible (150 companies x 4 = 600 vs ~1,284 projects, needs >=350); and the hub slim-index was sized at "~120 KB at 600 entries" when measurement gives 786 KB raw / 173 KB gzip at the confirmed 1,284 scope, which is 3.6x the curriculum.ts chunk the perf audit called P0, so the index is redesigned as a separately-fetched static JSON asset rather than a JS import. Two P0 gaps remain open: project completion is specified as an ungated toggle worth 40 XP feeding certificates, which rebuilds bug 0.1 on a new surface while the graded modes have unused graders; and Project/ProjectStep have no source field at all while CaseCompany and ConceptScenario both mandate one, so hallucinated tool paths and healthy/unhealthy thresholds would pass validation since every assertion is structural. Also recorded: section 11 mode percentages are an n=34 sample extrapolated to 454 then 642 while using the vocabulary of measurement, and are now labelled as estimates. NONE of it is applied yet, code untouched pending review.)
...
3. Current: 642 lessons · 158 glossary terms · 157 tools · 24 tracks · 21 categories
...
9. Practice projects layer (Stage 8, Session 73-86): **🎉 STAGE 8.3a IS FULLY COMPLETE — all 24 tracks in the locked priority order are done.** `/projects` hub live, 803 projects across the library (Phase 0+1 pilot + Technical SEO Mastery + AI Search Optimization + On-Page SEO Mastery + AI-First Marketer + Off-Page SEO Mastery + B2B Marketing + Paid Ads Mastery + Data-Driven Marketer + CRO & Conversion Mastery + Analytics & Measurement Mastery + Email & Lifecycle Mastery + Solo Founder + Freelancer & Agency + Content Creator + Marketing Mental Models + Copywriting Mastery + Content Strategy Mastery + Growth Marketing Mastery + E-commerce Growth + Social Media Manager + Product Marketing Mastery + Brand Strategy Mastery + Psychology of Marketing + PR & Communications Mastery tracks). **Session 78: every project now opens in its own dedicated page at `/projects/[category]/[id]` (statically generated, `src/lib/projects/lookup.ts`), in a new tab, from BOTH the lesson page's Project List and the `/projects` hub** — `ProjectCard.tsx` has a `variant: "preview" | "full"` prop, `ProjectDrawer.tsx` (the old slide-over) is deleted. **Stage 8.3b is fully done** (both AI Search Optimization and On-Page SEO Mastery, 27/27 lessons). **🎉 STAGE 10 IS FULLY COMPLETE** (10.1: every one of 642 lessons now has 5 quiz questions, 3,210 total questions, `quizzes.ts` ~2.4MB; 10.2: `PASS_THRESHOLD` 0.75→0.8 in `Quiz.tsx`; 10.3: stale "4 questions" references fixed in AGENTS.md/README.md; 10.4: re-audited, 0 position-dependent options across 12,840 option strings — all UNPUSHED per owner instruction, "push once everything is done only"). **🎉 STAGE 11 IS COMPLETE** (Skill Map/Achievements/Resources UX pass). **🎉 STAGE 9.3'S NARROWED 8-CATEGORY SCOPE IS FULLY COMPLETE (Session 86)**: `fundamentals`, `seo`, `paid-ads`, `growth`, `social`, `product-marketing`, `ai-marketing`, `tools` all done. Everything else (`content`, `email`, `analytics`, `copywriting`, `cro`, `events-experiential`'s remaining 20 lessons, `affiliate-marketing`, `marketing-leadership`, `legal-compliance`) remains explicitly out of scope, unchanged from the owner's Session 85 directive. **NEXT UP: nothing queued in Stage 9.3** — check with the owner before starting any of the explicitly-out-of-scope categories, since that scope was deliberately narrowed twice already. Status of the rest of Stage 9: 9.4 (hygiene) done. 9.1 (career layer) partially done: `/portfolio` page shipped and live; the `calibration` deferred-verification infra and `/certificates` big-project wiring remain, real separate Phase-3 work. 9.2 (Related Concepts) deprioritized: `RelatedLessons.tsx` already auto-generates related content for all 642 lessons, so the manual field is optional enrichment. 9.3: 8/28 `events-experiential` lessons done, but that category is now OUT of the narrowed scope; `track-projects.ts` (track-level big projects) also only covers 1 of 24 tracks. Nav.tsx's Topics mega menu was rewritten TWICE in Session 86: first from CSS-columns masonry to an explicit responsive grid anchored to the full header (not the trigger button — see AGENTS.md Rule 75, the same overflow trap applies to any future wide dropdown), then a second time to a two-pane flyout (left sidebar of discipline tabs, right panel of that group's topics) matching an owner-supplied reference page, verified with Playwright across breakpoints/themes — the grid version is superseded, don't revert to it. **`scripts/get-category-batch-info.mjs` does not resolve `sourceCategory` and will overcount a category's remaining project-less lessons by however many cross-listed lessons it contains — cross-check any "remaining" list against `grep -n 'sourceCategory' src/lib/curriculum.ts` before trusting it (Session 86, see AGENTS.md Rule 76; this is exactly what inflated `fundamentals`' true remaining count of 19 to a reported 32).** **`main` and `stage-8-3a-paid-ads-mastery` are merged as of PR #16** — future work continues on the feature branch, needs another PR to reach `main` again. **A simulation stage's `lessonAnchor` must point at a real `##`/`###` heading, never a bolded prose line like `**Mistake 1: X**` — if a lesson names its mistakes as bold text under one `## Common Mistakes` heading, use that heading's slug for every stage referencing any of them (Session 85).** **A batch authoring agent can stall silently after finishing its MDX edits but before writing its scratch projects file, with no completion notification ever arriving — if a batch has had no scratch-file activity for well past its usual ~5-7min runtime, check MDX/scratch-file mtimes rather than waiting indefinitely, then relaunch scoped ONLY to the missing work (skip MDX if InAction inserts already landed, to avoid duplicates) (Session 85).** **URGENT, unresolved as of this line: the Vercel Image Optimization free-plan quota fix (`images.unoptimized: true` in `next.config.ts`) is committed on local `main` (`e78b342`) but NOT pushed to GitHub** — the environment's git identity lacks push access and the owner's `Surya8991` GitHub CLI token is invalid. Push it (or restore access) before the site burns more of the exceeded quota. **A `cappedTierPair` of `["mini", "core-feeds-big"]` means author only ONE per-lesson project (mini tier) — the `core-feeds-big` slot is a track-level big-project synthesis input tracked in `src/lib/track-projects.ts`, not a second per-lesson project (Session 85).** **`paidUpgradeNote` lives on `ToolStack`, never inside a `ToolRef` item — `tsc` catches this one (Session 85).** New `src/lib/projects/social.ts`/`tools.ts` category files exist as of Session 85 (`SOCIAL_PROJECTS`/`TOOLS_PROJECTS`, first projects in each). **The site's `lessonAnchor` slug algorithm (mirrored in `tests/projects-data.test.ts`'s `headingIdsFor()` and `scripts/audit-projects.mjs`) is a SIMPLIFIED approximation of real `rehype-slug`/`github-slugger`, not identical to it — a heading with a `/` collapses to a SINGLE hyphen on this site (`how-it-works-the-playbook`), where real `github-slugger` produces a double hyphen (`how-it-works--the-playbook`). When unsure, compute the slug from the site's own regex (`h.toLowerCase().replace(/[^\w\s-]/g, "").trim().replace(/\s+/g, "-")`), not from the `github-slugger` npm package directly (Session 85).** **Use PROJECTS_AUTHORING_GUIDE.md for every track, don't re-derive the process**, start with `node --import tsx scripts/get-track-batch-info.mjs <track-slug>` (some tracks span multiple categories — batch/merge per category within the track, not per track). Centrality/tier data for all 642 lessons in src/lib/projects-assignment.ts (generated). New `src/lib/projects/cro.ts` category file exists as of Session 85 (first projects in that category, `CRO_PROJECTS` export) — when a batch targets a brand-new category, create the stub file yourself first (`export const X_PROJECTS: Record<string, Project[]> = {}`) before running `merge-projects-batch.mjs`, it reads the target file and errors if it doesn't exist. **Simulation stages must never use `nextStageId: ""` for a terminal option — always the `"end"` sentinel; `tsc`/lint/the audit script do not catch this, only `npm test` does (Session 85).** **`archetype` must be a real `Archetype` union value, never a `ProjectMode` value like `"calibration"` — `tsc` catches this one, but only if you actually run it before merging (Session 85).** **`mode: "teardown"` must populate `teardownItems[]`, not `steps[]` — `audit-projects.mjs` catches this one (Session 85).** **Gotcha found twice now (Session 77), check for it on every future batch**: a lesson's top-level project key must be double-quoted (`"slug": [`, not `slug: [`) — both are valid JS but an unquoted key is invisible to `get-track-batch-info.mjs`'s and `audit-projects.mjs`'s regex-based key detection, so a lesson can silently look like it "already has projects" when checked by eye but get miscounted by tooling (or vice versa, look like it needs projects when it already has an unquoted set). Run `grep -nE '^\s{2}[a-zA-Z][a-zA-Z0-9-]*:\s*\[' src/lib/projects/*.ts` before trusting any "remaining lessons" count. See AGENTS.md Rule 58.
10. Visual redesign (Session 74): Home/Learn/Tracks/Projects/Tools/About use a new "field manual" type system (font-display/font-ui-sans/font-data Tailwind classes = Fraunces/Public Sans/IBM Plex Mono). Colors are UNCHANGED from the original (white bg, indigo #6366f1 accent) — explicit user decision, do not "fix" this back, it's correct as-is. Lesson-reading prose still uses Geist Sans.
11. Lesson page redesign (Session 75): docs-style layout (Stripe Docs/MDN/GitBook-inspired). Left thin scroll-spy ToC, one consolidated meta line under the title, Quiz/Projects/Notes as a matched accordion group, RelatedConcepts.tsx (new) renders lessonMeta.relatedConcepts as cards, Mark Complete/Bookmark/Share moved to a bottom action bar, no more "Back to {category}" link.
12. Concept scenarios (Stage 8.4, Session 76-77): `src/components/InAction.tsx` (global MDX component, registered in root mdx-components.tsx), embedded directly in lesson MDX right after the heading it illustrates. 66 scenarios live so far: 24 in Technical SEO Mastery's 12 lessons (Session 76) + 28 in AI Search Optimization's 14 lessons + 14 in On-Page SEO Mastery's 7 rewritten-this-session lessons (Session 77). Not build-time-injected (see AGENTS.md Rule 54 for why).
13. Project content architecture v2 (Session 80-81): `Project` type gained 9 optional fields (skills/prerequisites/terminology/keyQuestion/whatToLookFor/decision/professionalRecommendation/commonMistakes/keyTakeaway), see AGENTS.md Rules 63-65 for field shapes, page section order, and the hairline-divider visual language (not stacked card boxes). **`seo.ts` is fully migrated, 60/60 projects.** **NEXT UP: the other 8 category files** (ai-marketing, analytics, content, copywriting, email, fundamentals, growth, mental-models, paid-ads — 53 more projects), one category at a time per the owner's Session 80 decision.
```
17: 
18: **Do NOT:**
...
└── src/
    ...
    │
    ├── auth.ts                     ✅ Session 87 — NextAuth v5 config (Google), requireUser/requireAdmin/isAdminUser, session() allow-list + lazy DrizzleAdapter (fix wave)
    ├── server/
    │   └── db/                     ✅ Session 87 — schema.ts (users/accounts/sessions/verificationTokens), client.ts (lazy Proxy `db`/`getDb()`, fix wave), migrations/
    │
    ├── lib/session-cookie.ts       ✅ Session 87 fix wave — shared Auth.js cookie-name lookup (checks both `__Secure-` and non-prefixed names)
    ├── lib/quiz-keys.ts            ✅ Session 87 fix wave — quiz key-prefix constants moved out of quizzes.ts (Rule 41 bundle-size precedent)
    ├── lib/sync-client.ts          ✅ Session 87 — mergeSnapshots/pullAndMerge/pushNow/startAutoSync, user-scoped local timestamp (fix wave)
    ├── lib/progress-snapshot.ts    ✅ Session 87 — EXPORT_KEYS/ALLOWED_KEY_PREFIXES/collectAllKeys/restoreAllKeys, shared by /settings + sync
    │
    ├── app/account/                ✅ Session 87 — page.tsx + AccountClient.tsx (profile, sessions, delete account)
    ├── app/login/                  ✅ Session 87 — page.tsx + SignInButton.tsx
    │
...
| 79 | 2026-08-13 | **Execution.** Fixed Mermaid flowchart rendering size/blank space issues (defs bypass + requestAnimationFrame fallback). Ran a full project audit and upgraded DiagramBlock's funnel, bars, timeline, cycle, and flow diagrams with foreignObject text-wrapping to prevent text overflow. |
| 71 | 2026-08-12 | **Execution.** Implemented PROJECTS_PLAN.md Stages 0-7 (9 commits, merged to main via PR #11). Stage 0: deleted /api/groq, disabled sync UI, gated certificates, try/catch in ThemeToggle/OnboardingModal/LessonNotes. Stage 1: quiz integrity family (8 items). Stage 2: sourceCategory resolution, storage safety (7 items). Stage 3: CATEGORY_INDEX for Nav (-48KB gzip), lazy DOMPurify/PostHog, /learn collapse. Stage 3b: 18 tests across 6 suites. Stage 5: stale counts fixed, dead GitHub URL, fake badges removed, 306 copy artifacts cleaned. Stage 6: focus-visible, reduced-motion, aria-live, aria-pressed, 13 nested main landmarks. Stage 7: track quiz pagination (10/page + sessionStorage), empty states, mobile StreakBadge, certificate print CSS, onboarding improvements. New files: `src/lib/storage-utils.ts`, `src/components/StorageWarning.tsx`, `tests/data-validation.test.ts`, `tests/quiz-shuffle.test.ts`, `tests/integrity-regression.test.ts`. Deleted: `src/app/api/groq/route.ts`. |
| 70 | 2026-08-11 | **Planning session, no feature code.** Wrote `PROJECTS_PLAN.md`, the high-priority roadmap for hands-on projects on all 642 lessons + a `/projects` hub + per-concept scenarios in lessons. Four-agent survey of 454 lessons established a six-mode project model (diagnostic, simulation, build, teardown, drill, calibration) plus an explicit `no-project` verdict. Full mechanical audit of all 642 lesson files recorded as a backlog in plan section 12. **Found ~120 issues total, 13 P0, none fixed.** 5 interlocking integrity bugs (plan 0.1-0.1f), a 20-item lesson backlog (plan s12), and ~94 from a five-lens adversarial code audit (plan s14): unauthenticated /api/groq LLM proxy with no callers, /api/sync-proxy sharing one global KV key across all users, 3 unguarded localStorage calls that crash every page, certificates printable at 0%, fundamentals capped at 27/40 and 2 achievements mathematically unreachable, 48KB gzip of curriculum.ts on every route: `TrackLessonList.tsx` completes lessons with no quiz check; `certificates/[slug]` has **no eligibility gate at all** (printable at 0%); `Quiz.tsx` reveals answers then allows retry; the track quiz marks failed lessons complete at an 80% pooled threshold; `markIncomplete()` does not reverse XP. `AGENTS.md` Rule 24 corrected (it had documented this as intentional and now contradicted Rule 25); Rules 36-39 added. Bug fix specified in plan 0.1, **not yet applied**. |
| 88c | 2026-09-17 | **Executed the audit backlog (10 commits, `development-branch`).** SEO/AEO: per-route self-canonicals (canonical bug), title double-suffix fix (30 pages), `/llms.txt` manifest, sitemap 931→1,734 URLs + unfrozen lastmod, `robots.ts` disallow + `noindex` on 8 personal routes, BreadcrumbList/LearningResource JSON-LD on compare/cheat-sheet/project pages. Perf: `CATEGORY_INDEX` → standalone `category-index.ts` literal (removes ~148 KB curriculum from every route; drift-guarded by a new test). Privacy: PostHog `autocapture:false`+`respect_dnt:true`; removed dead newsletter code. A11y: theme-aware error box, labeled search/notes inputs, AA-contrast Callout labels. Feature: certificate **name field** (`cert-name.ts`, syncs). Cleanup: compare "coming soon" copy, README route table. New files: `src/lib/category-index.ts`, `src/lib/cert-name.ts`, `src/app/llms.txt/route.ts`, `src/app/certificates/[slug]/layout.tsx`, `tests/category-index.test.ts`. AGENTS.md Rule 79. **66/66 tests, tsc clean.** Full tracker: `IMPROVEMENT_PLAN.md`. Queued LAST: 642-lesson interlink map. |
| 88b | 2026-09-17 | **8-perspective audit → `IMPROVEMENT_PLAN.md` + first SEO/meta fixes.** Audited learner/SEO/perf/a11y/privacy/business/maintainer/AEO-GEO. Fixed (P0): sitewide canonical→homepage bug (removed root-layout `alternates.canonical`, added self-canonicals to homepage + 18 routes) and title double-suffix on 30 pages (`X \| Marketing Academy \| Marketing Academy` → single). AGENTS.md Rule 79 added. Verified in-browser, `tsc`/lint clean. Remaining items (incl. a 642-lesson interlink map queued LAST) tracked in `IMPROVEMENT_PLAN.md`. |
| 88 | 2026-09-17 | **Quiz UX: always-available retake + results on pass.** `Quiz.tsx`, (1) the "Retake quiz" button now renders on EVERY finished screen (pass, fail, and the mount-restored "Already Passed!" screen), previously it was gated behind `!alreadyPassed` so a passed learner had no way to retake short of a full progress reset; `handleRetry()` also resets the in-memory `alreadyPassed` flag but never clears the persisted pass flag, so a retake can never re-lock an earned lesson. (2) The full per-question review (correct option + explanation) now renders on the pass screen too, not just fail, gated on `!alreadyPassed && selections.length === totalQuestions` and extracted into a shared `reviewList` used by both branches. Verified in-browser on `/learn/seo/core-web-vitals` (5/5 pass shows review + Retake; reload shows "Already Passed!" + Retake; Retake restarts at Q1). `tsc`/lint clean. Merged to `main` via [PR #26](https://github.com/Surya8991/Marketing-Academy/pull/26) (merge commit `3b1bcad`). Docs: README quizzes feature line, AGENTS.md Rule 25 (retake/review notes), this row. |
...
### legal-compliance: 7 -> 12 lessons (the 5 orphaned MDX files now registered)
The 5 lessons drafted earlier this session (`can-spam-email-compliance`, `sms-tcpa-compliance`, `cookie-consent-tracking-law`, `sweepstakes-contest-law`, `dark-patterns-ftc-enforcement`) were wired into `src/lib/curriculum.ts` (5 new `LessonRef` entries) and `src/lib/quizzes.ts` (5 new 4-question quiz blocks, 20 questions, keyed `legal-compliance/{slug}`). `npx tsc --noEmit` clean after the edits. Remaining planned topics for legal-compliance to reach the original 13-lesson target: `coppa-marketing-to-children`, `ai-generated-content-legal-risk`, `price-advertising-drip-pricing-law`, `greenwashing-ftc-green-guides`, `native-advertising-disclosure`, plus 2 more (data processing agreements, right to be forgotten, not yet started).
...
## Session 64, 2026-07-22 (Thin-category saturation, part 2: events-experiential, affiliate-marketing, marketing-leadership, legal-compliance all -> 20)

**Finished the saturation plan from Session 63. 44 lesson MDX files (already drafted on disk by earlier background agents) were wired into `curriculum.ts` and `quizzes.ts` in one pass across the 4 remaining thin categories.**

- `events-experiential`: 8 -> 20 lessons. Added `event-marketing-tech-stack`, `pop-up-retail-experiences`, `conference-speaking-thought-leadership`, `event-pr-media-relations`, `user-conference-strategy`, `roadshow-multi-city-events`, `community-meetup-programs`, `event-swag-branded-merch`, `crisis-planning-live-events`, `account-based-events-abm`, `post-event-content-repurposing`, `event-budgeting-hidden-costs`.
- `affiliate-marketing`: 8 -> 20 lessons. Added `affiliate-content-sites-seo`, `coupon-deal-site-affiliates`, `saas-affiliate-programs`, `affiliate-attribution-multi-touch`, `affiliate-onboarding-enablement`, `sub-affiliate-networks`, `affiliate-vs-paid-media-mix`, `b2b-partner-referral-programs`, `affiliate-cookie-tracking-technical`, `international-affiliate-programs`, `affiliate-program-negotiation`, `de-influencing-affiliate-trust`.
- `marketing-leadership`: 8 -> 20 lessons. Added `marketing-budget-planning`, `marketing-org-design`, `agency-vs-inhouse-decision`, `marketing-ops-function`, `cross-functional-alignment-sales`, `hiring-marketing-generalists-specialists`, `marketing-team-remote-distributed`, `personal-brand-marketing-leader`, `marketing-leadership-layoffs-restructuring`, `marketing-mentorship-coaching`, `board-reporting-marketing`, `first-90-days-cmo`.
- `legal-compliance`: 12 -> 20 lessons. Added `coppa-marketing-to-children`, `native-advertising-disclosure`, `drip-pricing-junk-fees`, `ai-generated-content-ftc`, `email-list-purchasing-legality`, `loyalty-referral-program-law`, `health-wellness-marketing-claims`, `biometric-location-privacy-marketing`.

- `src/lib/curriculum.ts` — 44 new `LessonRef` entries appended to the 4 categories' `lessons` arrays.
- `src/lib/quizzes.ts` — 44 new 4-question quiz blocks (176 questions) keyed `{category}/{slug}`.
- README.md, PROJECT_LOG.md, `src/app/opengraph-image.tsx` lesson counts updated 532 -> 576 per Rule 23 and Rule 27. All 21 categories now saturated to parity (20 lessons, or MDX-count-matched for the larger original categories).
- Session 65: README.md, PROJECT_LOG.md, `src/app/opengraph-image.tsx` lesson counts updated 576 -> 624. All 6 "thin" categories (mental-models, pr-communications, events-experiential, affiliate-marketing, marketing-leadership, legal-compliance) saturated 20 -> 28, matching the mid-range of the original categories.
...
## Session 89, 2026-09-17 (Nav declutter + superadmin visibility fix + today's-pushed em-dash removal + profile/settings polish)

**Owner-requested cleanup pass on the day's shipped work (#23-#30 accounts/profile/admin/email feature set): fix the superadmin admin link, declutter the Nav actions bar into one user menu, show only the last 5 recent-activity rows on /profile behind a "Show more" button, give /settings the site masthead, refresh a stale tool count, and strip the em dashes added in the day's pushes.**

### Superadmin admin-dashboard visibility (bug fix)
- A `SUPERADMIN_EMAILS` address that is not also in `ADMIN_EMAILS` got `isAdmin: false` on the session, so the Nav "Admin" link stayed hidden and `requireAdmin()` / `/admin` would have redirected it away. Fixed in `src/auth.ts`'s `session()` callback: `isAdmin` is now `isAdminUser(u) || isSuperAdminEmail(u.email)`, i.e. superadmin is a strict superset of admin. `isSuperAdmin` stays separate and still gates the superadmin-only surfaces (`/admin/users`, the "Manage users" panel). Nothing is DB-persisted, so Rule 80 holds. See the Rule 80 annotation added this session.

### Nav declutter (owner: "Nav Structure is too cluttered")
- The desktop actions bar dropped the three standalone icon links (Profile, Bookmarks, Settings) and folded them, plus Account & sync, Admin, and Sign in/out, into a single user menu (the avatar button, `hidden md:block`, the mobile hamburger already carries all of it). The bar is now Streak, Command palette, Theme, Start Learning, and the one user-menu icon. Profile/Bookmarks/Settings work for guests too, so the menu shows for everyone; the account/auth rows adapt to auth state. `src/components/Nav.tsx`.
- Mobile menu rebuilt (owner: "should we show that much topics on mobile, collapse sections by default"): Start Learning CTA on top, then Topics / Learn / Resources as native `<details>` accordions collapsed by default (all 21 topics behind one tap instead of a 42-item inline dump), then direct Profile/Bookmarks/Settings/About + auth rows. Accordion chevrons reuse the existing `.details-chevron` global convention (rotate 90deg on open), not a Tailwind `group-open` variant (which does not generate in this v4 setup). Panel height dropped from a long scroll to ~407px collapsed.
- Footer: added the new `/profile` hub to the Learn column (`src/components/Footer.tsx`).

### /profile recent activity (owner: "show only last 5, hidden by a Show more button")
- `ProfileClient.tsx` now renders the first 5 recent-activity rows and a "Show N more" / "Show less" toggle when there are more than 5 (the underlying `recentActivity` is capped at 15 in `profile-stats.ts`).
- Activity heatmap enlarged (owner: "make this big"): `ActivityHeatmap.tsx` cells went from 11px to 15px on mobile / 20px on sm+ (gaps 3px to 4-5px), so the 18-week grid reads clearly instead of as tiny dots.

### /settings polish
- Added the shared `PageMasthead` ("MARKETING ACADEMY · SETTINGS") and moved the heading to the site display font, matching /profile and /admin. Functionality unchanged.

### Counts refresh outside lessons/projects (owner request)
- Homepage and /about already derive counts dynamically (`uniqueLessonCount()` = 642, `TOOLS.length`, `GLOSSARY_TERMS.length`, `TRACKS.length`), and the OG image / llms.txt hardcoded lesson counts (642) were already current. The one drift: README cited "157 tools" in 4 places; real count is 159. Updated all 4. Lessons (642), glossary (158), tracks (24), categories (21) verified correct.

### Em-dash removal (owner: "remove the em dashes added in today's pushed")
- Swept every em dash added in the day's commits (base `e85e04a`), 437 across 361 lines in 58 files, using smart context-aware punctuation (heading/label patterns to a colon, appositive/clause breaks to a comma, a couple of email strings to parentheses). Only lines newly authored today were touched: pre-existing em dashes (including the 100%-identical docs merely moved into `docs/` this session, and older comments in files edited today) were left untouched by diffing each file against its pre-today content. Repo-wide the em dash is not otherwise banned (existing content keeps it); this reflects the owner's preference for the day's output.

### Verification
- `npx tsc --noEmit` clean; `eslint` 0 errors (1 pre-existing warning in `OnboardingModal.tsx`); `npm test` 75/75; `npm run build` passes.
- Browser check (dev server): Nav user menu opens with Profile/Bookmarks/Settings consolidated, /settings shows the new masthead, /profile renders the dashboard with the correct 642 lesson total. (The local `/api/auth/session` 500s are environmental, no auth env is set in local dev, and are unrelated to these changes.)
